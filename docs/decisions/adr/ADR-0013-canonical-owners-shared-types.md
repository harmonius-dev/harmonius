...
The [2026-04-12 design review](../../design/design-review.md) §2.2 catalogued nine shared types
that were defined or redefined in multiple files with subtly different APIs:
...
Foundational primitives existed (e.g., `core-runtime/primitives.md` after the P0 work) but no
single index recorded ownership. New design docs continued to introduce duplicates because no
authoritative list existed to consult.

## Decision

A single normative registry —
[`docs/design/canonical-owners.md`](../../design/canonical-owners.md) — names the sole owner
for every shared type. Each row carries a status:
...
The registry covers container primitives, IDs, errors, frame and time types, hot-reload
protocol, graph runtime, data-systems shared types, rendering shared types, audio and
networking shared types, and tooling/platform shared types.

Every design and integration document MUST reference, not redefine, types listed in the
registry. Adding a duplicate is a folder-rule violation. Discovering a duplicate creates a
backlog issue under [BL-0001..BL-0014](../../backlog/index.md).
