...
Traversal iterators **SHALL** support trees up to 256 levels deep without heap allocation. For trees
exceeding the fixed stack depth, the iterator **SHALL** transparently fall back to heap allocation
rather than failing. The engine **SHALL** report a diagnostic warning when traversal encounters a
hierarchy deeper than 128 levels.

- **Derived from:** [F-1.2.2](../../features/core-runtime/scene-and-transforms.md)
- **Rationale:** Extremely deep hierarchies may indicate design issues; graceful fallback prevents
  crashes while the warning helps identify problematic content.
- **Verification:** Unit test: traverse a 256-level hierarchy and verify zero heap allocations.
  Traverse a 300-level hierarchy and verify it completes correctly via fallback. Verify the warning
  fires at depth 129.

## Parent-Child Relationships

### R-1.2.3 Cascading Lifecycle Propagation

The engine **SHALL** recursively despawn all descendants when a parent entity is despawned, with an
...
- **Verification:** Benchmark: propagate 2 million entities across 4 cores and verify total time is
  under 1 ms. Modify a local transform in frame N and verify the world-space result is available to
  all systems in frame N (not N+1).

## Dirty Tracking

### R-1.2.5 Transform Dirty Tracking

The engine **SHALL** use ECS change detection to mark transforms as dirty and skip world-space
...
- **Rationale:** Brute-force spatial queries are infeasible at scale; an incrementally updated index
  amortizes update costs across frames.
- **Verification:** Benchmark: insert 2 million entities, perform 100 frustum queries per frame and
  verify all complete within 1 ms total. Verify incremental update cost is proportional to moved
