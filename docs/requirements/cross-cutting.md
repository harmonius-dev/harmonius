...
1. **R-X.1.1** — The engine **SHALL** define per-system frame time budgets for 30fps (33.3ms), 60fps
   ...
   the profiling API (F-15.5.1). When total frame time exceeds the target, the budget arbiter
   **SHALL** request load-shedding from systems in priority order: VFX first, then rendering quality
   (dynamic resolution), then AI LOD, then physics substep reduction.
...
2. **R-X.1.2** — The engine **SHALL** partition GPU memory into subsystem pools with configurable
   ...
   streaming system (F-12.5.7) **SHALL** coordinate with the rendering system to evict textures
   before render targets when under global VRAM pressure.
...
1. **R-X.4.1** — The engine **SHALL** initialize systems in this order: (1) Platform (windowing,
   ...
   without the failed system if it is optional (per F-13.1.9 modular systems).
...
2. **R-X.5.2** — All systems that use random number generation (procedural generation, loot tables,
   AI decision variance, particle spawning, combat damage rolls) **SHALL** use seeded pseudo-random
   number generators derived from a per-session master seed. The master seed **SHALL** be logged and
...
1. **R-X.6.1** — Asset hot reload **SHALL** only take effect at designated sync points between
   ...
   affected by hot reload **SHALL** be notified via the ECS observer system (F-1.1.30) with the set
   of changed asset handles, enabling them to refresh their derived state (re-bake lightmaps,
   rebuild NavMesh tiles, recompile shaders, refresh UI layout).
...
| ID      | Derived From                                                           |
|---------|------------------------------------------------------------------------|
| R-X.7.1 | F-13.3.1 (Save/Load Serialization), F-1.4.7 (Full Scene Serialization) |
...
1. **R-X.9.1** — The engine **SHALL** use a single wind system architecture serving all consumers
   ...
   entities into a 3D grid. Foliage shaders (F-3.3.4), cloth simulation (F-4.7.5), particle forces
   (F-11.1.1), hair dynamics (F-9.5.6), and audio wind effects **SHALL** all sample from this shared
   wind field texture. No system **SHALL** maintain a separate wind model.
...
2. **R-X.9.2** — The engine **SHALL** provide a single cloth simulation system and a single ragdoll
   ...
   (F-4.7.x XPBD solver) with the animation domain (F-9.5.x) providing character-specific authoring,
   LOD, and skinning integration. Ragdoll **SHALL** be owned by the physics domain (F-4.3.5
   joint-based ragdoll) with the animation domain (F-9.3.4) providing blend weights, partial
   ragdoll, and animated-to-ragdoll transitions. No duplicate simulation system **SHALL** exist.
   Animation requirements that reference cloth or ragdoll **SHALL** delegate to the physics system
   for simulation and provide only the animation-side integration layer.
...
3. **R-X.9.3** — Voice chat audio packets **SHALL** pass through exactly one jitter buffer — the
   audio-domain jitter buffer (F-5.5.2). The network-domain jitter buffer (F-8.4.6) **SHALL NOT**
   process voice packets; it **SHALL** only buffer game state updates. Voice packets **SHALL** be
   identified by a channel type field in the packet header, and the network layer **SHALL** route
   voice packets directly to the audio system's jitter buffer, bypassing the game state buffer
   entirely.
...
4. **R-X.9.4** — The shader graph system (F-15.8.5) **SHALL** compile material and shader graphs to
   HLSL, which DXC (Direct3D Shader Compiler) compiles to DXIL and SPIR-V, and Metal Shader
   ...
   **SHALL NOT** be generated as web is not a target platform. All shader compilation paths in the
   ...
   SPIRV-Cross dependencies. DXC and Metal Shader Converter are C++ libraries accessed via cxx.rs
   FFI bindings.
...
5. **R-X.9.5** — All user-facing authoring surfaces — gameplay logic, material graphs, animation
   ...
   scripting languages, DSLs, or command-line tools. The formula system (F-13.7.4) **SHALL** use
   visual formula nodes within the logic graph, not a textual DSL. The hot reload system (F-12.4.4)
...
6. **R-X.9.6** — Each simulation domain (physics, AI, audio, animation) **SHALL** expose
   accessibility hooks that the UI accessibility system (F-10.6.x) can consume. Audio **SHALL**
   provide visual directional indicators for spatial sound cues when the user enables visual audio
   mode. Physics **SHALL** provide simplified difficulty modes that reduce timing precision
   requirements for motor-impaired players. AI **SHALL** respect per-player difficulty modifiers for
   perception ranges, reaction times, and accuracy. The game framework **SHALL** expose a unified
   accessibility settings panel that configures all cross-domain accessibility hooks from a single
   location.
...
| ID       | Derived From                 |
|----------|------------------------------|
| R-X.10.1 | Cross-domain review findings |
...
1. **R-X.10.1** — The engine **SHALL** resolve feature ownership for capabilities that appear in
   ...
   data generation and simulation; (2) the core-runtime spatial indexing features (F-1.9.x) are the
   sole spatial acceleration structure — scene-and-transforms spatial features (F-1.2.6, F-1.2.7)
   delegate to them; (3) ECS observers, command buffers, and resources are defined in
...
1. **R-X.11.1** — The engine **SHALL** complete all skeletal animation evaluation (keyframe
   ...
   exceeded, the animation LOD system (F-9.1.10) **SHALL** reduce update rates and bone counts for
   distant entities before dropping any visible animation.
   ...
   - **Derived From:** F-9.1.10 (Animation LOD), F-9.1.5 (Instanced Skeletal Animation), R-X.1.1
     (Frame Time Budget)
...
1. **R-X.12.1** — The engine **SHALL** render the full UI (widget tree, text, overlays, nameplates,
   minimaps) within 2ms of GPU time per frame at 60fps. The UI batching system (F-10.4.1) **SHALL**
...
2. **R-X.12.2** — The 2D physics simulation (F-10.5.10) **SHALL** produce bit-identical results
   across all supported platforms given identical initial state and input sequence, using the same
...
1. **R-X.13.1** — The engine **SHALL** allocate a configurable GPU compute budget for VFX (default:
   2ms at 60fps). The VFX budget manager (F-11.6.5) **SHALL** track per-emitter compute cost and
   enforce the budget by scaling down lower-priority effects before hero effects are affected. Total
   alive particle count **SHALL** be capped at a configurable limit (default: 500,000).
...
2. **R-X.13.2** — The engine **SHALL** cap the total number of active decals at a configurable limit
   ...
   limits are exceeded, the decal lifecycle system (F-11.2.4) **SHALL** reclaim the oldest
   low-priority decals first.
...
1. **R-X.14.1** — The engine **SHALL** import and process assets at a sustained rate of at least 100
   assets per minute during batch import (F-12.1.5) on a machine with 16 CPU cores. Incremental
   builds (F-12.3.4) **SHALL** process only changed assets, achieving at least 10x faster build
   times than full rebuilds for changes affecting less than 1% of the asset library.
   ...
   - **Derived From:** F-12.1.5 (Batch Import), F-12.3.4 (Incremental Builds), F-12.3.3 (Hash-Based
     Caching)
2. **R-X.14.2** — The engine **SHALL** complete asset hot reload (F-12.4.2) — from file change
   detection through in-memory swap — within 2 seconds for textures, 3 seconds for meshes, and 500ms
   for logic graphs. Shader hot reload (F-12.4.3) **SHALL** complete recompilation and pipeline swap
   within 5 seconds.
...
1. **R-X.15.1** — The engine **SHALL** achieve at least 80% of raw sequential disk bandwidth through
   the platform-native async I/O layer (F-14.3.5, F-12.5.2) on all supported platforms. GPU direct
   storage (F-12.5.3) **SHALL** achieve at least 3 GB/s throughput on NVMe hardware where supported.
...
2. **R-X.15.2** — The engine **SHALL** successfully generate a crash dump for at least 99% of
   ...
   - **Verification:** Inject 100 crashes (null deref, stack overflow, heap corruption, GPU hang);
     ...
     subsequent launch.
   - **Derived From:** F-14.4.1 (Crash Handler), F-14.4.2 (Symbol Upload), F-14.4.3 (Crash
     Aggregation)
