...
| ID       | Derived From                                                    |
|----------|-----------------------------------------------------------------|
| R-12.2.1 | [F-12.2.1](../../features/content-pipeline/asset-processing.md) |
| R-12.2.2 | [F-12.2.2](../../features/content-pipeline/asset-processing.md) |
| R-12.2.3 | [F-12.2.3](../../features/content-pipeline/asset-processing.md) |
| R-12.2.4 | [F-12.2.4](../../features/content-pipeline/asset-processing.md) |
| R-12.2.5 | [F-12.2.5](../../features/content-pipeline/asset-processing.md) |
| R-12.2.6 | [F-12.2.6](../../features/content-pipeline/asset-processing.md) |
| R-12.2.7 | [F-12.2.7](../../features/content-pipeline/asset-processing.md) |
| R-12.2.8 | [F-12.2.8](../../features/content-pipeline/asset-processing.md) |
| R-12.2.9 | [F-12.2.9](../../features/content-pipeline/asset-processing.md) |

1. **R-12.2.1** — The engine **SHALL** compress imported textures offline into GPU-native
   block-compressed formats (BC7/BC6H for desktop and console, ASTC for mobile and Apple Silicon,
   ETC2 as mobile fallback) with quality levels and format selection driven by import presets and
   ...
   - **Verification:** Compress a test texture for each target platform and confirm the output
     format matches the platform override table; compare PSNR against the quality threshold for each
     preset.
...
4. **R-12.2.4** — The engine **SHALL** reorder triangles and vertices within each meshlet to
   ...
   - **Verification:** Measure ACMR (average cache miss ratio) before and after optimization and
     confirm improvement; measure overdraw ratio and confirm reduction compared to unoptimized
     order.
...
7. **R-12.2.7** — The engine **SHALL** compile visual shader graph assets into clean, human-readable
   HLSL source files. Each graph node **SHALL** emit an HLSL function; the compiler **SHALL**
   resolve input connections, generate typed local variables, and compose node functions into
   complete shader entry points. Custom nodes **SHALL** be user-authored HLSL snippets with function
   signatures called with resolved inputs. Generated .hlsl files **SHALL** contain no template
   markers, preprocessor hacks, or non-standard extensions -- they **SHALL** be fully compatible
   with HLSL syntax highlighting and HLSL Tools features (IntelliSense, error squiggles,
   go-to-definition) in any IDE. Comments **SHALL** annotate which graph node produced each code
   section. Only reachable shader variants identified through static analysis **SHALL** be
   generated.
   ...
   - **Verification:** Generate HLSL from a shader graph with 20+ nodes; open in Visual Studio with
     HLSL Tools; verify syntax highlighting, IntelliSense, and error squiggles work correctly.
     Verify no template markers appear in the output. Verify comments trace each section to its
     graph node.
...
9. **R-12.2.9** — The engine **SHALL** compile generated HLSL source files (R-12.2.7) into
   ...
   **SHALL** report the original HLSL line number AND the graph node that generated it, enabling
   ...
   - **Verification:** Compile a shader with dead code and constants, confirm dead code is removed
     and constants are folded; confirm reflected binding layouts match the source HLSL; validate
     each output format on its target API.
