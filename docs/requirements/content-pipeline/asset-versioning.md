...
2. **R-12.7.2** — The engine **SHALL** group related assets into compressed bundles using LZ4 for
   ...
   - **Rationale:** Compressed bundles with partial decompression reduce storage and bandwidth while
     avoiding the latency penalty of full-bundle decompression.
   - **Verification:** Create a bundle with 10 assets; extract the 5th without decompressing the
     other 9; confirm the manifest lists all 10 with correct offsets; confirm LZ4 at runtime and
     Zstd at distribution time.
...
3. **R-12.7.3** — The engine **SHALL** diff any two asset versions at the structural level, showing
   ...
   - **Rationale:** Structural diffs let artists understand what changed at the semantic level
     rather than inspecting raw binary differences.
   - **Verification:** Diff two logic graph versions with an added node and confirm it is
     highlighted; diff two mesh versions and confirm vertex count deltas are shown; diff two
     textures and confirm visual side-by-side is rendered.
4. **R-12.7.4** — The engine **SHALL** merge two divergent asset versions against a common ancestor,
   ...
   - **Rationale:** Structural three-way merge with a Git merge driver enables concurrent artist
     edits without forcing pessimistic file locking.
   - **Verification:** Branch an asset, make non-overlapping changes on each branch, merge, and
     confirm automatic success; make overlapping changes and confirm the conflict is presented with
     resolution options.
5. **R-12.7.5** — The engine **SHALL** automatically resolve non-ambiguous merge conflicts using
   ...
   resolution and strategy configurable per asset type and per project.
   - **Rationale:** Conservative automatic resolution reduces manual merge workload while preserving
     safety through fallback to manual resolution.
   - **Verification:** Create a merge with independent property changes and confirm automatic
     resolution; create one with ambiguous changes and confirm fallback to manual; change the
     strategy per asset type and confirm it applies.
...
6. **R-12.7.6** — The engine **SHALL** provide a spreadsheet-style editor for gameplay data tables
   ...
   bulk operations, real-time cell validation, CSV import/export, and per-cell undo/redo.
   - **Rationale:** A spreadsheet interface matches designers' mental model for tabular data and
     provides bulk-editing capabilities needed for large gameplay data sets.
   - **Verification:** Open a data table with 100 rows; sort, filter, search; edit a cell and
     confirm undo/redo; enter an invalid value and confirm a constraint violation is highlighted;
     export to CSV and reimport confirming round-trip fidelity.
7. **R-12.7.7** — The engine **SHALL** provide a dedicated visual inspector for every asset type (3D
   ...
   - **Verification:** Open one asset of each type and confirm the correct visual inspector is
     displayed; confirm no raw text or code view is available for any asset type.

## Version Control Integration

8. **R-12.7.8** — The engine **SHALL** track binary assets via Git LFS with a custom merge driver
   that invokes the engine's structural merge system for three-way merges, support optional
   ...
   - **Verification:** Commit a binary asset via Git LFS; merge two branches with conflicting
     ...
     and confirm the asset browser shows lock owner and modified state.
