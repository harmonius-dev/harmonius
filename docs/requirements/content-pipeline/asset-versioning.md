...
and modified elements specific to each asset type (nodes/connections for logic graphs, vertex
count/bounding box for meshes, rows/columns for data tables, visual side-by-side with difference
...
## R-12.7.7 Universal Asset Inspector

The engine **SHALL** provide a dedicated visual inspector for every asset type (3D preview with
wireframe for meshes, channel/mip/histogram view for textures, timeline with curve editor for
