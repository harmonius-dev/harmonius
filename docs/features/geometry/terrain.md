...
| ID       | Feature |
|----------|----------------------------------- |
| F-3.2.15 | Three-Tier Coordinate Spaces |
| F-3.2.16 | Cube-Sphere Chunk Projection |

1. **F-3.2.15** — Three coordinate spaces coexist to support multi-planet worlds: `Chunk-local`
   (per-chunk flat grid, used by voxel editing and storage), `Planet-local` (per-planet Cartesian
   origin at the planet center, used by physics, gameplay, and AI), and `Universe` (global Euclidean
   frame connecting planet origins, used by orbital transforms and rendering of distant bodies).
   Each planet is an ECS world (F-1.1.34) with its own spatial index and coordinate origin; the
   universe is a lightweight top-level world containing planet entities with orbital transforms.
   Inter-planetary travel migrates an entity (F-1.1.35) and applies a one-time Planet A → Universe →
   Planet B coordinate conversion.
   - **Deps:** F-1.1.34 (Multiple World Support), F-1.1.35 (Entity Migration Between Worlds),
     F-3.2.7 (Large World Coordinate Support), F-4.1.15 (Multi-Planet Physics Worlds)
2. **F-3.2.16** — Planets are subdivided into 6 cube faces, each face a flat grid of chunks. The
   vertex shader performs two transforms: chunk-local Cartesian to planet surface via normalized
   cube projection (curving the flat grid onto the sphere by the `planet_radius`), then planet
   surface to universe position via the planet's orbital transform. Shared edge vertices and
   averaged normals handle cube-face seams so the visible surface is continuous across face
   boundaries. Physics and collision detection continue in flat chunk-local Cartesian space — the
   curvature is purely a vertex shader concern.
   - **Deps:** F-3.2.15, F-3.2.1 (Tile-Based Heightfield)

## 2D Tilemap Geometry

| ID       | Feature |
|----------|----------------------- |
| F-3.2.17 | Chunked 2D Tilemap |
| F-3.2.18 | Auto-Tiling and Wang Tiles |
| F-3.2.19 | Tilemap Collision Generation |

1. **F-3.2.17** — Chunked 2D tilemap storage organizes tiles into fixed-size chunks (e.g., 16x16).
   Each tile records an atlas index plus flags (flip X, flip Y, rotate, collision type). Multiple
   tilemap layers (background, midground, foreground) compose per chunk with per-chunk dirty flags
   for incremental rendering updates. Chunks stream in and out based on camera position, supporting
   infinite and procedural tilemaps using the same streaming infrastructure as heightfield terrain.
   - **Deps:** F-3.2.1 (Tile-Based Streaming), F-1.8.1 (Async I/O)
2. **F-3.2.18** — Auto-tiling automatically selects tile variants from neighbor connectivity via
   4-bit cardinal or 8-bit cardinal-plus-diagonal bitmasks. Supports Wang tile and blob tileset
   conventions so that designers paint terrain regions without manually picking variants. The
   bitmask lookup resolves transitions between biomes, borders, and tile types.
   - **Deps:** F-3.2.17
3. **F-3.2.19** — Tile collision generation produces 2D physics shapes per chunk from tile flags:
   solid tiles become rectangle colliders, slope tiles become triangle colliders, and one-way
   platform tiles pass through from below. Each chunk registers a compound 2D collider in the 2D
   physics BVH (F-4.1.16). Collision is rebuilt incrementally only for chunks whose tiles changed.
   - **Deps:** F-3.2.17, F-4.1.16 (2D Rigid Body Mode)
