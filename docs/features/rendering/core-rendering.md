...
- **Dependencies:** None
- **Platform notes:** Mobile: max 16 dynamic lights per tile; simplified attenuation.
  Switch: max 32 lights per tile; handheld reduces to 16. Desktop: full quality, up to
  256 lights per tile. High-end: unlimited lights per tile with stochastic sampling.
...
- **Platform notes:** Mobile: runs at reduced dispatch granularity to fit tile-based GPU
  budgets. Switch: full GPU culling in docked mode; handheld may batch fewer meshlets per
  dispatch. Desktop/High-end: full quality, no restrictions.
...
- **Dependencies:** None
- **Platform notes:** All platforms: full quality, no degradation. Lightweight compute
  pass with minimal bandwidth cost on tile-based mobile GPUs.
...
- **Platform notes:** Mobile: HZB at quarter resolution; phase 2 may be skipped under
  frame budget pressure. Switch: half-resolution HZB; both phases active. Desktop/High-end:
  full-resolution HZB with both phases.
...
- **Dependencies:** None
- **Platform notes:** All platforms: full quality. Reverse-Z requires clearing depth to 0
  instead of 1, supported on Metal, D3D12, and Vulkan. Older mobile Vulkan drivers may
  need viewport transform workaround.
...
individually in back-to-front sorted order (see F-2.4.5).

- **Requirements:** R-2.3.7
- **Dependencies:** F-2.3.2, F-2.3.3, F-2.3.4
- **Platform notes:** Mobile: instance buffer sized for lower object counts (~10k
  meshlets); indirect draw requires Vulkan 1.1+ or Metal family GPU 3+. Switch: full
  indirect draw support; handheld caps meshlet budget. Desktop/High-end: no limits.
...
- **Platform notes:** Mobile: off-screen targets should use memoryless storage where
  possible to avoid bandwidth on tile-based GPUs. All platforms: full support.
...
- **Platform notes:** Mobile: static cubemaps only; max 128x128 per face; ASTC
  compressed. Switch: one dynamic face update per frame; 256x256 per face. Desktop:
  full dynamic cubemaps at 512x512. High-end: 1024x1024 per face, multiple dynamic
  cubemaps per frame.
...
- **Dependencies:** F-2.3.8
- **Platform notes:** Mobile: max 1 scene capture active; quarter-resolution; simplified
  lighting (forward only, no shadows). Switch: max 2 captures; half-resolution.
  Desktop: configurable count and resolution. High-end: unlimited at full resolution.
...
- **Dependencies:** None
- **Platform notes:** Mobile: aggressive range 50-75% of native; targets 30 fps.
  Switch: 50-100% handheld (720p native), 60-100% docked (1080p native).
  Desktop: 67-100% with upscaling to native. High-end: 50-100% with TSR/DLSS to 4K.
...
- **Platform notes:** Mobile: preintegrated SSS LUT only (no screen-space blur); no RT
  SSS. Switch: screen-space SSS at half resolution. Desktop: full screen-space SSS.
  High-end: RT subsurface scattering with multi-sample profiles.
...
- **Dependencies:** None
- **Platform notes:** Mobile: alpha test breaks tile-based hidden surface removal on
  some GPUs; use sparingly and prefer opaque proxies at distance. All other platforms:
