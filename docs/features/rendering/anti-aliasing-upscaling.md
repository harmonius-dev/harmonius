...
- **Dependencies:** None
- **Platform notes:** Mobile: available but costly due to history buffer bandwidth; prefer FXAA as
...
- **Platform notes:** Mobile: heuristic TSR from 50% to native; quality mode only. Switch: TSR from
...
- **Dependencies:** None
- **Platform notes:** Mobile: default AA method; FXAA 3.11 quality preset 10 (low). Switch: FXAA
...
- **Platform notes:** Mobile: 2x MSAA with on-chip tile resolve (efficient on tile-based GPUs); 4x
  only on high-end iOS devices. Switch: 2x MSAA. Desktop: 2x or 4x MSAA. High-end: 4x MSAA (rarely
...
- **Dependencies:** None
- **Platform notes:** Mobile: effective bandwidth saver on tile-based GPUs; recommended for 1080p+
...
- **Platform notes:** Mobile: FSR 1.0 spatial-only upscaling available; no DLSS/XeSS. Switch 2: DLSS
...
- **Platform notes:** Mobile: disabled; insufficient compute and memory for frame synthesis. Switch
  2: single-frame DLSS frame gen at 30-to-60 fps target. Desktop: single-frame generation (DLSS 3 /
...
- **Platform notes:** Mobile: basic frame pacing only (no reflex-style pipeline). Switch: frame
  pacing with v-sync alignment. Desktop: NVIDIA Reflex / AMD Anti-Lag integration for sub-frame
  input sampling. High-end: full Reflex + frame gen latency compensation.
