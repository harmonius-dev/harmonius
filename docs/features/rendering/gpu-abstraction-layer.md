...
| ID      | Feature                               | Requirements |
|---------|---------------------------------------|--------------|
| F-2.1.4 | Metal Backend (Swift-to-C-to-Bindgen) | R-2.1.4      |

1. **F-2.1.4** — Metal GPU backend implemented as a Swift library exposing a C-compatible API
   surface via `@_cdecl` functions. Rust consumes the C header through bindgen, wrapping the raw
...
1. **F-2.1.5** — Direct3D 12 backend using C-compatible COM header bindings generated by bindgen.
...
| ID      | Feature                       | Requirements |
|---------|-------------------------------|--------------|
| F-2.1.6 | Vulkan Backend (C-to-Bindgen) | R-2.1.6      |

1. **F-2.1.6** — Vulkan backend generated from vulkan.h via bindgen. Safe Rust wrappers encapsulate
