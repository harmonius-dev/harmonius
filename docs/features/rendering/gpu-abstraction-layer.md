...
1. **F-2.1.1** — Defines the top-level Rust trait that all GPU backends implement, using associated
   ...
   - **Platform:** Each backend provides a concrete type that satisfies the trait bounds.
2. **F-2.1.2** — Trait-based command buffer encoding supporting graphics, compute, and copy
   ...
   - **Deps:** F-2.1.1
   - **Platform:** Maps to MTLCommandBuffer on Metal, ID3D12GraphicsCommandList on D3D12,
...
3. **F-2.1.3** — Unified pipeline state object covering shader stages, vertex layout, blend state,
   ...
   - **Deps:** F-2.1.1
   - **Platform:** Mobile: pipeline state cache warmed at load time to avoid hitching on tile-based
     GPUs where PSO creation is expensive. All platforms: full quality.
...
1. **F-2.1.6** — Vulkan backend using `ash` (thin Vulkan bindings). Safe Rust wrappers encapsulate
   ...
   is enabled in debug builds for runtime error checking.
   - **Deps:** F-2.1.1
   - **Platform:** Windows and Linux. Vulkan loader discovered at runtime. MoltenVK is not used —
...
1. **F-2.1.7** — GPU heap sub-allocator that carves typed buffer and texture regions from large
   ...
   counts.
   - **Deps:** F-2.1.1
   - **Platform:** Allocation strategy respects per-backend alignment requirements (256 B on D3D12,
     variable on Vulkan, page-aligned on Metal).
2. **F-2.1.8** — CPU-side shadow state tracker that records current pipeline, binding, and render
   ...
   - **Deps:** F-2.1.2
   - **Platform:** All platforms: full quality. Especially important on mobile where driver overhead
     per state change is proportionally higher due to lower CPU clocks.
3. **F-2.1.9** — Automatic resource barrier batching and reordering within a command buffer.
   ...
   - **Deps:** F-2.1.8
   - **Platform:** Maps to resource barriers on D3D12, pipeline barriers on Vulkan, and is a no-op
     on Metal where the driver manages hazard tracking.
4. **F-2.1.10** — Backend-level support for GPU work graphs enabling GPU-driven dispatch without CPU
   ...
   - **Platform:** D3D12 uses native Work Graphs API. Metal and Vulkan use compute-based emulation
...
5. **F-2.1.11** — Runtime feature emulation layer that provides consistent API behavior when a
   backend lacks native support for a capability. Emulated paths are selected at device creation
   time based on capability queries, with no runtime branching in the hot path.
   - **Deps:** F-2.1.1
   - **Platform:** Emulated features include work graphs on Vulkan/Metal, mesh shaders on older
     Vulkan drivers, and enhanced barriers on D3D12 feature level < 12.2.
6. **F-2.1.12** — Hardware timestamp queries, pipeline statistics, and occupancy counters exposed
   ...
   - **Deps:** F-2.1.1
   - **Platform:** Metal uses MTLCounterSampleBuffer. D3D12 uses ID3D12QueryHeap with
...
1. **F-2.1.13** — CPU-side emulation of D3D12 work graphs via indirect dispatch chains in the task
   ...
   those buffers and dispatch additional GPU work via `ExecuteIndirect`. The emulation path
   integrates with the unified game loop graph (F-14.3.14) as task graph nodes, enabling
   work-graph-style GPU scheduling on all platforms without native hardware support.
   - **Deps:** F-2.1.10, F-2.1.11, F-14.3.14
   - **Platform:** D3D12 uses native Work Graphs API when available. Metal and Vulkan use the
     emulation path with `ExecuteIndirect` / `vkCmdDispatchIndirect` / Metal indirect command
     buffers. Emulated path is selected at device creation time.
2. **F-2.1.14** — All GPU resources (buffers, textures, samplers, pipeline states, acceleration
   ...
   pointers (device addresses, CPU pointers to mapped memory) appear in any public API type.
   Internal raw pointers are confined to backend implementations behind safe handle-based wrappers.
   - **Deps:** F-2.1.1, F-2.1.7
