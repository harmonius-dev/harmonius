...
- **Platform notes:** iOS requires Xcode provisioning profiles. Android requires developer mode
  enabled. Console devkits require network configuration per manufacturer guidelines.

### F-15.14.3 Certification Compliance Checker

Automated validation of platform certification requirements before submission. Checks include:
...
- **Platform notes:** Certification rules differ per platform and are updated by platform holders
...
- **Platform notes:** iOS signing requires Xcode command-line tools. macOS notarization requires
  internet access and an Apple Developer account. Android signing uses apksigner or jarsigner.
  Windows EV signing may require a hardware token (USB dongle) for CI.
...
- **Platform notes:** .msi generation requires WiX Toolset on Windows. .deb packaging requires
  dpkg-deb. Flatpak requires flatpak-builder. SteamOS verification uses Valve's compatibility
  testing tools.
