...
- **Platform notes:** Mobile limits ORCA neighbor count to 4 (vs. 12 on desktop) and uses a smaller
...
pillars, and terrain edges. Operates as a final correction layer after ORCA, handling geometry that
is not represented as other agents.

- **Requirements:** R-7.2.2
- **Dependencies:** F-7.2.1
- **Platform notes:** Mobile uses 2 feeler rays (vs. 5 on desktop) for reduced raycast cost. Low-LOD
...
Provides a library of composable steering primitives: seek, flee, arrive (with deceleration radius),
wander (constrained random heading), pursuit (predictive interception), and evade (predictive
escape). Each returns a steering force vector that is blended by a priority or weighted-sum
combiner.
...
- **Platform notes:** Mobile limits active steering layers to 3 (vs. 5 on desktop) to reduce
  per-agent blending cost.

## Formation & Group Movement

### F-7.2.5 Formation Movement

Assigns agents to slots in a parameterized formation shape (line, wedge, column, circle, custom). A
formation leader drives the group along the path while followers maintain their offsets using
arrival steering, automatically adjusting spacing when terrain narrows or the formation is partially
destroyed.

- **Requirements:** R-7.2.5
- **Dependencies:** F-7.2.3, F-7.2.1
- **Platform notes:** Mobile limits formation size to 4 slots (vs. 8+ on desktop). Custom formation
...
- **Dependencies:** F-7.2.3
- **Platform notes:** Group size limited by the platform crowd budget (see F-7.7.4). Centroid
