...
1. **F-14.4.1** — Install a process-wide crash handler that intercepts unhandled exceptions,
   ...
   - **Platform:** Windows uses `SetUnhandledExceptionFilter` and `MiniDumpWriteDump` via COM
     ...
     `objc2`; Linux uses a `SIGSEGV`/`SIGABRT` signal handler with `/proc/self/maps` parsing and
     `ptrace`-based stack capture. Out-of-process crash capture is preferred on all platforms to
     avoid corruption in the faulting process.
