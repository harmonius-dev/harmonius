...
1. **F-15.13.1** — Visual string table editor with ICU patterns, TM, CSV
2. **F-15.13.2** — Locale preview, validation, pseudo-localization, RTL
3. **F-15.13.3** — XLIFF workflow, TMS integration, review, string locks
...
1. **F-15.19.1** — Auto-generated API reference from Reflect metadata
2. **F-15.19.2** — Logic graph node documentation with port docs
3. **F-15.19.3** — Interactive in-editor tutorials with spotlight
4. **F-15.19.4** — Embedded video player with chapter links
5. **F-15.19.5** — Contextual help tooltips and "What's This?" mode
6. **F-15.19.6** — Sample projects and template library per genre
7. **F-15.19.7** — Inline code examples compiled as CI doc-tests
...
1. **ICU data** — Ship CLDR data as an engine asset. Plural rules for all CLDR locales.
2. **RTL locales** — Arabic, Hebrew, Urdu, Persian. Mirror all widget layouts.
3. **CJK support** — Font fallback chains required for Chinese, Japanese, Korean. Large glyph
   atlases.
4. **Text shaping** — Delegate to platform text shaping: DirectWrite (Windows), CoreText (macOS),
   HarfBuzz (Linux).
5. **XLIFF 2.1** — Industry standard for translation interchange. All major TMS tools support it.
6. **Pseudo-localization** — No platform dependencies. Pure string transform.
...
1. **API reference** — Generated by CI. Deployed to self-hosted AWS S3 + CloudFront.
2. **In-editor help** — Bundled with engine distribution. Updated independently via asset download.
3. **Search index** — Trigram index fits in memory (< 50 MB for full engine API).
4. **Video streaming** — Streamed from AWS S3. Cached locally in player cache (F-14.5.9).
5. **Versioning** — Each engine release produces an independent doc snapshot. Old versions remain
   accessible.
...
| Test                           | Req       |
|--------------------------------|-----------|
| `test_icu_plural_en`           | R-15.13.1 |
| `test_icu_plural_ar`           | R-15.13.1 |
| `test_icu_validate_ok`         | R-15.13.1 |
| `test_icu_validate_err`        | R-15.13.1 |
| `test_icu_format`              | R-15.13.1 |
| `test_icu_extract_vars`        | R-15.13.1 |
| `test_set_locked_entry`        | R-15.13.3 |
| `test_unlock_edit`             | R-15.13.3 |
| `test_source_change_detection` | R-15.13.3 |
| `test_tm_fuzzy_match`          | R-15.13.1 |
| `test_tm_exact_match`          | R-15.13.1 |
| `test_xliff_roundtrip`         | R-15.13.3 |
| `test_xliff_malformed`         | R-15.13.3 |
| `test_csv_roundtrip`           | R-15.13.1 |
| `test_csv_empty_translations`  | R-15.13.1 |
| `test_pseudo_accented`         | R-15.13.2 |
| `test_pseudo_padded`           | R-15.13.2 |
| `test_pseudo_bracketed`        | R-15.13.2 |
| `test_validate_missing`        | R-15.13.2 |
| `test_validate_overflow`       | R-15.13.2 |
| `test_validate_rtl`            | R-15.13.2 |
| `test_validate_interpolation`  | R-15.13.2 |
| `test_bidi_resolve`            | R-15.13.2 |
| `test_font_fallback`           | R-15.13.1 |
| `test_font_runs`               | R-15.13.1 |
| `test_reflect_extract_type`    | R-15.19.1 |
| `test_reflect_extract_methods` | R-15.19.1 |
| `test_search_index_query`      | R-15.19.1 |
| `test_versioned_store`         | R-15.19.1 |
| `test_node_doc_completeness`   | R-15.19.2 |
| `test_tutorial_advance`        | R-15.19.3 |
| `test_tutorial_pause_resume`   | R-15.19.3 |
| `test_help_lookup`             | R-15.19.5 |
| `test_help_localized`          | R-15.19.5 |

1. **`test_icu_plural_en`** — English: 1=one, else=other. Verify select().
2. **`test_icu_plural_ar`** — Arabic: 6 plural forms (zero,one,two,few,many,other).
3. **`test_icu_validate_ok`** — Valid pattern "{count, plural, one {# item} other {# items}}"
   parses.
4. **`test_icu_validate_err`** — Missing "other" category returns MissingOtherCategory.
5. **`test_icu_format`** — Format pattern with args, verify output.
6. **`test_icu_extract_vars`** — Extract variable names from pattern.
7. **`test_set_locked_entry`** — Editing a locked entry returns EntryLocked.
8. **`test_unlock_edit`** — Unlock, edit, re-lock succeeds.
9. **`test_source_change_detection`** — Change source text, verify source_dirty set.
10. **`test_tm_fuzzy_match`** — Index 100 pairs, query similar string, verify similarity > 0.6.
11. **`test_tm_exact_match`** — Exact match returns similarity 1.0.
12. **`test_xliff_roundtrip`** — Encode then decode, verify identical data.
13. **`test_xliff_malformed`** — Malformed XML returns MalformedXml error.
14. **`test_csv_roundtrip`** — Encode then decode, verify all data preserved.
15. **`test_csv_empty_translations`** — Empty cells round-trip correctly.
16. **`test_pseudo_accented`** — All ASCII replaced with accented equivalents.
17. **`test_pseudo_padded`** — Output is ~1.3x length of input.
18. **`test_pseudo_bracketed`** — Output wrapped in brackets.
19. **`test_validate_missing`** — 5 of 10 strings missing -> 5 findings.
20. **`test_validate_overflow`** — Long German string exceeds widget -> finding.
21. **`test_validate_rtl`** — Arabic text with LTR widget -> RTL finding.
22. **`test_validate_interpolation`** — Mismatched variables -> finding.
23. **`test_bidi_resolve`** — Mixed LTR/RTL text, verify run order.
24. **`test_font_fallback`** — CJK codepoint falls through to CJK font.
25. **`test_font_runs`** — Mixed Latin/CJK string, verify 2 runs.
26. **`test_reflect_extract_type`** — Register type, extract ApiEntry, verify fields.
27. **`test_reflect_extract_methods`** — Extract methods with doc comments.
28. **`test_search_index_query`** — Index 100 types, query substring, verify hits.
29. **`test_versioned_store`** — Store two versions, fetch page from each.
30. **`test_node_doc_completeness`** — Node missing description -> finding.
31. **`test_tutorial_advance`** — Trigger completion event, verify step advances.
32. **`test_tutorial_pause_resume`** — Pause and resume preserves step index.
33. **`test_help_lookup`** — Lookup by widget_type_id returns correct entry.
34. **`test_help_localized`** — Lookup in "ja-JP" returns Japanese help.

### Integration Tests

| Test                        | Req       |
|-----------------------------|-----------|
| `test_full_locale_switch`   | R-15.13.2 |
| `test_pseudo_all_visible`   | R-15.13.2 |
| `test_xliff_crowdin_compat` | R-15.13.3 |
| `test_api_ref_generation`   | R-15.19.1 |
| `test_help_panel_500ms`     | R-15.19.1 |
| `test_tooltip_200ms`        | R-15.19.5 |
| `test_tutorial_end_to_end`  | R-15.19.3 |
| `test_doc_test_compile`     | R-15.19.7 |

1. **`test_full_locale_switch`** — Switch locale, verify all widgets re-layout.
2. **`test_pseudo_all_visible`** — Enable pseudo, verify no plain ASCII in rendered UI.
3. **`test_xliff_crowdin_compat`** — Export XLIFF, import into Crowdin, re-export, import back.
4. **`test_api_ref_generation`** — Generate full API ref, verify all types present.
5. **`test_help_panel_500ms`** — Open help panel, verify < 500 ms latency.
6. **`test_tooltip_200ms`** — Hover tooltip, verify < 200 ms latency.
7. **`test_tutorial_end_to_end`** — Run Getting Started tutorial, verify completion.
8. **`test_doc_test_compile`** — All code examples compile against current engine.
