...
4. **TC-2.3.10.6.3** `test_pak_cross_backend_build` -- Build D3D12, Vulkan, and Metal paks from the
   same sources.
...
3. **TC-2.3.10.5.B2** -- On-demand compile via dxc subprocess. Pass if < 200 ms.
