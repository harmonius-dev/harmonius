...
tessellation) are authored in GLSL and compiled via the standard naga pipeline (see
[constraints.md](../constraints.md) — Shader Pipeline).

| Platform | GPU Backend | Shader path |
|----------|-------------|-------------|
| Windows | Vulkan | GLSL → SPIR-V via `naga` |
| macOS / iOS | Vulkan | GLSL → SPIR-V → SPIR-V via `naga` + `naga` |
| Linux | Vulkan | GLSL → SPIR-V via `naga` |
...
casting) are authored in GLSL and compiled via the standard naga pipeline.
