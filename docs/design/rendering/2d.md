...
Each pass is a render graph node registered via `GraphBuilder::add_raster_pass` or
`GraphBuilder::add_compute_pass` (see [render-pipeline.md](render-pipeline.md) for the render graph
API). Passes declare read/write dependencies; the graph compiler determines execution order and
inserts barriers. The 2D pass subgraph is a child of the per-view render graph and composes with 3D
passes via the viewport stack.

### Integration with Render Graph

2D rendering uses the **same** shared render graph as 3D — there is no parallel graph. Every pass
registers via the shared `GraphBuilder` and the graph compiler handles barriers, resource aliasing,
queue assignment, and execution order exactly as for 3D passes.

```rust
fn register_2d_passes(
    graph: &mut GraphBuilder,
    camera_view: &RenderView,
    color_target: ResourceHandle,
    instance_buf: ResourceHandle,
    atlas: ResourceHandle,
    tile_vertex_ssbo: ResourceHandle,
    light_map: ResourceHandle,
) {
    // Tilemap compute pass: generate tile vertices from tile SSBO.
    graph
        .add_compute_pass("tilemap_compute")
        .read_storage(atlas)
        .write_storage(tile_vertex_ssbo)
        .execute(|enc, ctx| {
            enc.bind_compute_pipeline(ctx.pipelines.tilemap_gen);
            enc.dispatch(ctx.tile_groups_x, ctx.tile_groups_y, 1);
        })
        .finish();

    // Sprite batch raster pass: instanced draw of sorted sprites.
    graph
        .add_raster_pass("sprite_batch")
        .read_storage(instance_buf)
        .read_texture(atlas)
        .write_color(0, color_target)
        .execute(|enc, ctx| {
            enc.bind_graphics_pipeline(ctx.pipelines.sprite_batch);
            enc.draw(4, ctx.sprite_count, 0, 0);
        })
        .finish();

    // 2D lights compute pass: shadow + accumulation.
    graph
        .add_compute_pass("2d_light_accumulate")
        .read_storage(instance_buf)
        .write_storage(light_map)
        .execute(|enc, ctx| {
            enc.bind_compute_pipeline(ctx.pipelines.light_accumulate_2d);
            enc.dispatch(ctx.light_groups_x, ctx.light_groups_y, 1);
        })
        .finish();

    // UI overlay raster pass: renders HUD on top of world.
    graph
        .add_raster_pass("ui_overlay_2d")
        .write_color(0, color_target)
        .execute(|enc, ctx| {
            enc.bind_graphics_pipeline(ctx.pipelines.ui_overlay);
            enc.draw(4, ctx.ui_quad_count, 0, 0);
        })
        .finish();
}
```

Each 2D pass is a first-class node alongside 3D passes. Barriers between 2D and 3D work are inserted
by the shared graph compiler; 2.5D and split-screen compositions just submit more passes into the
same `GraphBuilder`.

### 2D Transform Interpolation

2D rendering uses `PreviousGlobalTransform2D` exactly the way 3D rendering uses
`InterpolatedTransform`. The scene-transforms subsystem maintains `PreviousGlobalTransform2D` per
entity each fixed-step, and `SpriteExtractSystem` reads both `GlobalTransform2D` and
`PreviousGlobalTransform2D` during extract to compute the interpolated transform that is uploaded to
the sprite instance buffer:
...
pub fn extract_sprite_instance(
    curr: &GlobalTransform2D,
    prev: &PreviousGlobalTransform2D,
    alpha: f32,
) -> Mat3 {
    lerp_transform_2d(&prev.matrix, &curr.matrix, alpha)
}
```

`alpha` comes from the shared `FrameContext::interp_alpha` (see
[../core-runtime/game-loop.md](../core-runtime/game-loop.md)). `PreviousGlobalTransform2D` is
defined canonically in [../core-runtime/scene-transforms.md](../core-runtime/scene-transforms.md);
this doc consumes it read-only during extract.

### Unified Camera System

2D and 3D cameras are **separate types** (`Camera2D` and `CameraComponent`) wrapping a shared
`Projection` enum defined in [rendering-core.md](rendering-core.md). There is no `CameraBase`
superclass; both cameras share:

1. `Projection` — `Perspective | Orthographic | PhysicalCine | Orthographic2D`
2. `RenderLayerMask` — the same newtype for render-layer filtering
3. `RenderView` and `ViewUniform` — both cameras build these shared types for the render graph

`Camera2D::layer_mask` is a `RenderLayerMask` (see `RenderLayerMask` in rendering-core.md RF-14).
Keeping the concrete camera types separate lets 2D cameras carry 2D-specific state (`dead_zone`,
`pixel_perfect`, `zoom`) without burdening 3D cameras.

### 2D Light Lookup Hot Path

The light accumulation pass iterates visible 2D lights once per frame and must look up per-light
shadow textures and parameters. The previous stub used a hash lookup;
**the hot path now uses `SortedVecMap<Entity, Light2DState>`** to match the engine-wide "no HashMap
on hot paths" rule (see [../core-runtime/primitives.md](../core-runtime/primitives.md)):

```rust
pub struct Light2DState {
    pub shadow_tex: ResourceHandle,
    pub params: Light2DParams,
}

pub struct Light2DBuffer {
    /// Hot-path indexed lookup for light accumulation. Replaces HashMap.
    pub by_entity: SortedVecMap<Entity, Light2DState>,
    pub visible: SmallVec<[Entity; 32]>,
}
```

`LightAccumulationPass` iterates `visible` and indexes `by_entity` — O(log n) per lookup,
cache-friendly iteration, deterministic order.
...
tessellation) are authored in GLSL and compiled via the standard glslc CLI pipeline (see
[constraints.md](../constraints.md) — Shader Pipeline).

| Platform | GPU Backend | Shader path |
|----------|-------------|-------------|
| Windows | Vulkan | GLSL → SPIR-V via `glslc` |
| macOS / iOS | Vulkan | GLSL → SPIR-V → SPIR-V via `glslc` + `glslc` |
| Linux | Vulkan | GLSL → SPIR-V via `glslc` |

No platform has hand-written SPIR-V or GLSL. All shader variants are compiled at asset-processing
time.
...
1. **Mobile** — iOS uses Vulkan via `ash`; Android uses Vulkan via `ash`. Tile atlas capped
   at 1024×1024 to fit within Vulkan mobile tier-1 texture limits.
2. **Desktop** — Windows uses Vulkan via `windows-rs`; macOS uses Vulkan; Linux uses Vulkan.
...
- **Vulkan staging buffers (Windows)**: tilemap chunk GPU buffers are uploaded via Vulkan staging
  buffers for reduced CPU overhead on chunk streaming.
- **Vulkan staging buffers (Apple)**: tilemap chunk data streams via Vulkan staging buffers for
  equivalent zero-copy GPU upload on Apple platforms.
...
Desktop). Add Switch and VR rows. Note platform-specific GPU backends (Vulkan) and any 2D-specific
adaptations.

### RF-12: GLSL shader pipeline

State that all 2D rendering shaders (sprite batch, tilemap compute, light map composite, shadow
casting) are authored in GLSL and compiled via the standard glslc CLI pipeline.
