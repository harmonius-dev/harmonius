...
- GPU compute via Vulkan compute shaders (GLSL compiled by naga).
- Fluid and cloth dispatch use compute command lists on the async compute queue when available.

**macOS:**

- GPU compute via Vulkan compute shaders (GLSL compiled by naga, converted by naga
  via C ABI).
- Command buffer completion handlers are GCD dispatch blocks. Controlled drain at poll point.

**Linux:**

- GPU compute via Vulkan compute shaders (GLSL compiled to SPIR-V by naga).
