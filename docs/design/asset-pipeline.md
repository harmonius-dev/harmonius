...
struct CookedMetadata {
  ...
  // Texture metadata
  rg::Format format = rg::Format::kUndefined;
  uint32_t width = 0;
  uint32_t height = 0;
...
class AssetCooker {
 public:
  ...
  // Cook batch (parallel)
  [[nodiscard]]
  std::vector<std::expected<CookedAsset, CookError>> CookBatch(std::span<const RawAsset> assets);

  // Register custom asset processor
  void RegisterProcessor(AssetType type, std::move_only_function<CookedAsset(const RawAsset&)> fn);
};
...
class MeshletBuilder {
 public:
  ...
  [[nodiscard]]
  MeshletData Build(std::span<const float> positions, std::span<const uint32_t> indices, uint32_t vertex_stride);
  ...
  [[nodiscard]]
  std::vector<MeshletData> BuildLodChain(std::span<const float> positions, std::span<const uint32_t> indices,
                                           uint32_t vertex_stride, uint32_t lod_count);
};
...
struct TextureCookOptions {
  rg::Format target_format = rg::Format::kBc7Unorm;  // default block-compressed
...
class TextureCompressor {
 public:
  [[nodiscard]]
  CookedAsset Compress(std::span<const uint8_t> rgba_data, uint32_t width, uint32_t height,
...
- **DirectStorage:** GPU-direct DMA requires sector alignment
- **Sparse texture pages:** tile granularity is typically 64KB
- **Platform-native async IO:** page-aligned reads avoid copy overhead (IOCP, dispatch_io, io_uring)
...
enum class StreamPriority : uint8_t {
  kCritical = 0,  // visible this frame, blocks rendering
  kHigh = 1,      // will be visible next few frames
  kNormal = 2,    // predictive prefetch
  kLow = 3,       // speculative background load
};
...
class StreamingScheduler {
 public:
  explicit StreamingScheduler(const BundleManifest& manifest, resource::PoolAllocator& pool,
  ...
  [[nodiscard]]
  std::vector<exec::TransferPassDesc> ProcessPending();

  // Handle eviction when pools are full
  void SetEvictionPolicy(std::move_only_function<std::vector<AssetId>(uint32_t needed)> policy);

  // Query residency state
  [[nodiscard]] bool IsResident(AssetId id) const;
  [[nodiscard]] float ResidencyRatio() const;  // fraction of requested assets loaded

  // Access the platform IO backend (compile-time selected)
  [[nodiscard]] IOBackend& IoBackend();
...
template <typename B>
concept IOBackendConcept = requires(B b, const B cb, std::string_view path, uint64_t offset, uint64_t size,
                                    gpu::ResourceHandle staging, uint64_t buf_offset) {
  // Submit async read from file to staging buffer
  { b.SubmitRead(path, offset, size, staging, buf_offset) } -> std::same_as<void>;
...
// No C++ standard library file IO is used; all backends use platform-native async IO.
class DispatchIOBackend {
  // Uses dispatch_io (Grand Central Dispatch) for async file reads on macOS
 public:
  void SubmitRead(std::string_view path, uint64_t file_offset, uint64_t size, gpu::ResourceHandle staging_buffer,
                  uint64_t buffer_offset);
  [[nodiscard]] std::vector<IOCompletion> PollCompletions();
};

class DirectStorageBackend {
  // Uses DirectStorage for GPU decompression pipeline on Windows (D3D12)
 public:
  void SubmitRead(std::string_view path, uint64_t file_offset, uint64_t size, gpu::ResourceHandle staging_buffer,
                  uint64_t buffer_offset);
  [[nodiscard]] std::vector<IOCompletion> PollCompletions();
...
class IocpBackend {
  // Uses I/O completion ports for async file reads on Windows (Vulkan)
 public:
  void SubmitRead(std::string_view path, uint64_t file_offset, uint64_t size, gpu::ResourceHandle staging_buffer,
                  uint64_t buffer_offset);
  [[nodiscard]] std::vector<IOCompletion> PollCompletions();
...
// Build-time selection — CMake aliases the concrete type based on GPU backend and platform
#if defined(HARMONIUS_BACKEND_METAL)
using IOBackend = DispatchIOBackend;
#elif defined(HARMONIUS_BACKEND_D3D12)
using IOBackend = DirectStorageBackend;
#elif defined(HARMONIUS_BACKEND_VULKAN) && defined(_WIN32)
using IOBackend = IocpBackend;
#elif defined(HARMONIUS_BACKEND_VULKAN) && defined(__linux__)
...
enum class AssetId : uint64_t { kInvalid = 0 };
...
class ResourceRegistry {
 public:
  explicit ResourceRegistry(gpu::Device& device, BindlessDescriptorHeap& heap);

  // Register a cooked asset's GPU resource and get a handle
  [[nodiscard]]
  AssetHandle RegisterAsset(AssetId id, gpu::ResourceHandle gpu_resource, const CookedMetadata& metadata);
  ...
  [[nodiscard]]
  std::expected<uint32_t, AssetError> ResolveDescriptorIndex(AssetHandle handle) const;

  // Resolve render graph ResourceHandle to asset
  [[nodiscard]]
  AssetHandle FindByRgResource(rg::ResourceHandle rg_handle) const;
...
flowchart TD
    ...
    subgraph rg_import["Render Graph Import"]
        Import["DeclareImported(<br/>  bindless_heap,<br/>  heap_handle<br/>)"]
    end
...
class BindlessDescriptorHeap {
 public:
  explicit BindlessDescriptorHeap(gpu::Device& device, uint32_t max_descriptors = 1'000'000);
  ...
  [[nodiscard]] gpu::ResourceHandle GpuHandle() const;

  // Import into render graph
  [[nodiscard]] rg::ResourceHandle ImportToGraph(rg::builder::GraphBuilder& builder) const;
...
sequenceDiagram
    ...
    Sched->>RG: InjectTransfer(staging to device)
...
```mermaid
flowchart LR
    A[AssetId] -->|ResourceRegistry| B[gpu::ResourceHandle]
    B --> C[rg::ResourceHandle]
    C -->|"Executor::bind_resource()"| D[gpu::ResourceHandle]
    D -->|BindlessHeap| E["uint32_t index"]
    E --> F["Shader: ResourceDescriptorHeap[index]"]
```
...
boundaries via `Executor::BindResource()`, and the graph continues executing with the
new resource.
