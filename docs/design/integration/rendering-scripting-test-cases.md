...
|----|------|-------|----------|-----|
| TC-IR-3.5.1.U1 | `codegen_hlsl` is pure | Same graph, 1000 runs | Byte-identical HLSL output | IR-3.5.1 |
| TC-IR-3.5.1.U2 | Arena HLSL handle roundtrip | Emit + resolve handle | HLSL bytes match source | IR-3.5.1 |
| TC-IR-3.5.1.U3 | PBR node to HLSL sort | 3-node PBR graph | Topological order emitted | IR-3.5.1 |
...
| ID | Test | Input | Expected | Req |
|----|------|-------|----------|-----|
| TC-IR-3.5.1.I1 | PBR graph emits HLSL | Albedo tex + metallic slider | Valid HLSL w/ `SurfaceOutput` | IR-3.5.1 |
| TC-IR-3.5.1.I2 | Unlit graph emits HLSL | Color constant only | HLSL with `Unlit` model | IR-3.5.1 |
| TC-IR-3.5.1.I3 | Normal map node emits TBN | Graph w/ normal sample | HLSL includes TBN transform | IR-3.5.1 |
| TC-IR-3.5.2.I1 | dxc produces DXIL | Valid PBR pixel HLSL | DXIL passes validation | IR-3.5.2 |
| TC-IR-3.5.2.I2 | dxc produces SPIR-V | Valid PBR pixel HLSL | SPIR-V passes `spirv-val` | IR-3.5.2 |
| TC-IR-3.5.2.I3 | MSC produces metallib | DXIL from dxc | metallib loads on Metal `[GPU]` | IR-3.5.2 |
...
| TC-IR-3.5.5.I1 | PP graph emits compute HLSL | Bloom threshold + blur | Valid compute HLSL | IR-3.5.5 |
| TC-IR-3.5.5.I2 | PP pass registers in graph | Compiled PP compute | Pass node in render graph | IR-3.5.5 |
| TC-IR-3.5.6.I1 | Effect graph emits kernels | Spawn+gravity+color | 4 compute kernels in HLSL | IR-3.5.6 |
...
| ID | Test | Input | Expected | Req |
|----|------|-------|----------|-----|
| TC-IR-3.5.1.N1 | `[NEG]` HLSL codegen error | Cyclic graph | Error node; prior handle kept | IR-3.5.1 |
| TC-IR-3.5.1.N2 | `[NEG]` Unknown node kind | Graph w/ unresolved node | Build fails with descriptive error | IR-3.5.1 |
| TC-IR-3.5.2.N1 | `[NEG]` dxc compile error | Syntactically invalid HLSL | Exit != 0; stderr captured | IR-3.5.2 |
| TC-IR-3.5.2.N2 | `[NEG]` dxc missing binary | CLI not installed | Graceful error; build fails | IR-3.5.2 |
| TC-IR-3.5.2.N3 | `[NEG]` MSC translation error | Malformed DXIL | Previous metallib retained | IR-3.5.2 |
| TC-IR-3.5.3.N1 | `[NEG]` Missing permutation | Unknown `PermutationKey` | Falls back to default permutation | IR-3.5.3 |
| TC-IR-3.5.4.N1 | `[NEG]` Hot reload compile fail | Invalid HLSL edit | Previous pipeline still renders | IR-3.5.4 |
| TC-IR-3.5.4.N2 | `[NEG]` Stale handle resolve | Freed + reused slot | `resolve` returns `None` | IR-3.5.4 |
| TC-IR-3.5.4.N3 | `[NEG]` dxc stderr to overlay | Compile error, 3 diagnostics | `Failed{3}` in ECS resource | IR-3.5.4 |
| TC-IR-3.5.5.N1 | `[NEG]` PP graph invalid binding | Missing input texture | Build fails; pass not registered | IR-3.5.5 |
| TC-IR-3.5.6.N1 | `[NEG]` Effect kernel compile fail | Invalid spawn HLSL | Effect disabled; warning emitted | IR-3.5.6 |
...
| TC-IR-3.5.2.B1 | Cold `dxc` compile single shader | < 500 ms | IR-3.5.2 |
| TC-IR-3.5.2.B2 | Cached `dxc` compile | < 1 ms | IR-3.5.2 |
