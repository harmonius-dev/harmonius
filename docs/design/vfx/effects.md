...
8. **F-15.8.5b** — Shader graph to GLSL compilation
...
3. A codegen backend: `GlslBackend` that fuses each lifecycle stage into a compute shader.
...
vocabulary, GLSL fusion, budget integration, and runtime effect subsystems (decals, screen effects,
weather, destruction).
...
visual graph. The compiler translates this into GLSL compute shaders that execute entirely on the
GPU.
...
4. **Event-driven.** Destruction VFX spawn via ECS observers. Weather transitions via `WeatherState`
   changes.
5. **Graph-to-shader.** Visual graph compiles to fused GLSL compute per emitter lifecycle stage.
...
/// Not a trait object. All custom node GLSL is compiled at middleman build time.
...
| GLSL codegen 128 nodes | < 10 ms | R-11.6.1 |
...
5. **Custom node compilation** -- Per-particle custom nodes must compile to GLSL, not bytecode.
