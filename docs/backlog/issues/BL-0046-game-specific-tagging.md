...
Per `user-stories/AGENTS.md` rule 8, game-specific stories must carry the `[game-specific]`
tag. The 2026-05 audit found this tag applied inconsistently — mostly only in
`game-framework/`. Many stories elsewhere are also game-specific (e.g., physics
`vehicle-physics.md`, animation `first-person.md` motion).
