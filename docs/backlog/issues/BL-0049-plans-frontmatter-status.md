...
All 141 plan body files carry `status: not_started` in their YAML frontmatter. Progress
stubs report 96 `code_complete`, 23 `started`, 12 `submitted`, 10 `not_started`. The
deslop pass added [plans/AGENTS.md](../../plans/AGENTS.md) clarifying that progress files
are the source of truth — but the contradiction in the plan-body files still creates noise.
...
A spot-check of 20 random plans shows consistent frontmatter; no plan body claims
`not_started` while its progress file says `code_complete`.
