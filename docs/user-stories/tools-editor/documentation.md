...
description, input/output port descriptions with types and valid ranges, and usage examples, **so
that** I can understand node behavior without consulting external documentation.
...
**As a** modder (P-24), **I want** node documentation accessible in the mod SDK graph editor, **so
that** I can build mod logic without access to engine source code or external docs.
