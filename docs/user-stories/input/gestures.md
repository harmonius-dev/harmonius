...
**As an** engine developer (P-26), **I want** double-tap recognition with configurable timing, **so
that** quick actions (auto-run) work.
...
**As an** engine developer (P-26), **I want** tap distance tolerance scaled with display DPI, **so
that** mobile vs desktop touchscreens are accommodated.
...
**As a** player (P-23), **I want** long press to initiate drag for inventory rearrangement, **so
that** item management works on touch.

## US-6.3.1.12 Test Gesture Recognition on Various DPIs
**As a** QA tester (P-19), **I want to** test gesture recognition on screens of different DPI, **so
that** tolerance scaling is validated.
...
**As a** QA tester (P-19), **I want to** test swipe recognition on phone, tablet, and trackpad, **so
that** all surfaces work correctly.
...
**As an** engine developer (P-26), **I want** two-finger rotation tracked with angle delta, **so
that** camera orbit and map rotation work.
...
**As a** player (P-23), **I want** touch input to work without gestures eating each other, **so
that** controls are reliable.
...
**As a** designer (P-5), **I want** conflict resolution strategies configurable in the editor, **so
that** gesture behavior is tunable.
...
**As an** engine developer (P-26), **I want** gesture resolution consistent across platforms, **so
that** behavior is predictable everywhere.
...
**As an** engine tester (P-27), **I want to** test gesture cancellation and failure paths, **so
that** error states are handled cleanly.
...
**As a** QA tester (P-19), **I want to** test custom gestures on phone, tablet, and trackpad, **so
that** all touch devices work.
