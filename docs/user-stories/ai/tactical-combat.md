...
protection angle, sight lines, flanking exposure, target distance, and objective proximity, **so
that** cover selection is context-aware.
...
**As a** player (P-23), **I want** a lone enemy to attack directly without attempting to flank, **so
that** flanking requires realistic squad coordination.
...
functional messages (target position, flanking call, retreat order) and plays bark audio, **so
that** communication is both functional and presentational.
