...
## US-13.7.1.1 Define Table Schemas With Typed Columns

**As a** gameplay director (P-3), **I want to** define gameplay data table schemas with typed
columns, constraints, and default values via the visual editor, **so that** data tables have
well-defined structures without writing code.
...
## US-13.7.4.1 Author Formulas as Visual Node Graphs

**As a** gameplay director (P-3), **I want to** compose formulas by connecting math nodes (add,
multiply, min, max, clamp, lerp, random range) in the logic graph editor, **so that** damage
calculations and XP requirements are authored visually.
...
**As a** designer (P-5), **I want to** recipes to require minimum skill levels and optional
discovery conditions before they appear, **so that** recipe availability reflects progression.
...
**As a** designer (P-5), **I want to** loot entries to have conditions (level range, quest state,
faction), **so that** drops are contextually relevant to the player.
...
**As a** designer (P-5), **I want to** asset lists to support per-platform overrides (lower-
resolution on console) and localization variants, **so that** assets adapt to target hardware.
