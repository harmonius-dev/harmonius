...
///
/// Excess counts first move to [`AnimationLodTier::ReducedBones`], then to
/// [`AnimationLodTier::Vat`] when a second fixed budget is exhausted (test contract).
#[must_use]
pub fn allocate_with_arena_overflow_policy(
    arena: &GpuArenaBuffer,
    requested_instances: u32,
    reduced_bones_capacity: u32,
    ...
    let take_reduced = remaining.min(reduced_bones_capacity);
