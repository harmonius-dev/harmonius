...
pub fn visual_binding_write_transform(
    ...
    let prev_mat = existing_current.map(|g| g.0).unwrap_or(newly_computed.0);
    (newly_computed, PreviousGlobalTransform(prev_mat))
}
