...
extern "C" EPError EPPlatformCreate(EPPlatformPtr* out_platform) {
    ...
    if (volkInitialize() != VK_SUCCESS) {
        return error(EP_E_UNSUPPORTED, "failed to initialize vulkan loader");
    }
    ...
    *out_platform = reinterpret_cast<EPPlatformPtr>(platform.release());
    return ok();
}
...
extern "C" EPError EPInstanceCreate(const EPInstanceDesc* desc, EPInstancePtr* out_instance) {
    ...
    ::vk::ApplicationInfo app_info{
        .pApplicationName = "Enthrall",
        .applicationVersion = VK_MAKE_VERSION(1, 0, 0),
        .pEngineName = "Enthrall",
...
extern "C" EPError EPInstanceEnumerateAdapters(EPInstancePtr instance, uint32_t* io_count,
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        adapter->ep_properties.backends = EP_BACKEND_VULKAN_BIT;
        ...
        // Feature detection
        uint32_t features = EP_FEATURE_COMPUTE_BIT;
        if (adapter->has_timeline_semaphore) features |= EP_FEATURE_TIMELINE_SEMAPHORE_BIT;
        if (adapter->has_mesh_shader) features |= EP_FEATURE_MESH_SHADER_BIT;
        if (adapter->has_ray_tracing) features |= EP_FEATURE_RAY_TRACING_BIT;
        ...
        adapter->ep_properties.features = static_cast<EPFeatureFlags>(features);
