...
extern "C" EPError EPDeviceCreate(EPAdapterPtr adapter_ptr, const EPDeviceDesc* desc,
    ...
    if (vmaCreateAllocator(&allocator_info, &allocator) != VK_SUCCESS) {
        return error(EP_E_OUT_OF_MEMORY, "failed to create VMA allocator");
    }
...
extern "C" EPError EPDeviceGetQueue(EPDevicePtr device_ptr, EPQueueType type, uint32_t index,
    ...
    *out_queue = reinterpret_cast<EPQueuePtr>(queue.release());
    return ok();
}
...
extern "C" EPError EPQueueSubmit(EPQueuePtr queue_ptr, const EPSubmitInfo* submit) {
...
extern "C" EPError EPQueueWaitIdle(EPQueuePtr queue_ptr) {
