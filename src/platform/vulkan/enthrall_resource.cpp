...
extern "C" EPError EPBufferCreate(EPDevicePtr device_ptr, const EPBufferDesc* desc, EPBufferPtr* out_buffer) {
    ...
    *out_buffer = reinterpret_cast<EPBufferPtr>(buffer.release());
    return ok();
}

extern "C" EPError EPBufferDestroy(EPBufferPtr buffer_ptr) {
    if (buffer_ptr) {
...
extern "C" EPError EPBufferMap(EPBufferPtr buffer_ptr, void** out_data) {
...
extern "C" EPError EPTextureCreate(EPDevicePtr device_ptr, const EPTextureDesc* desc, EPTexturePtr* out_texture) {
    ...
    // Create image view
    bool is_depth = desc->format == EP_FORMAT_D24S8 || desc->format == EP_FORMAT_D32_FLOAT;
    ...
    *out_texture = reinterpret_cast<EPTexturePtr>(texture.release());
    return ok();
}

extern "C" EPError EPTextureDestroy(EPTexturePtr texture_ptr) {
    if (texture_ptr) {
...
extern "C" EPError EPSamplerCreate(EPDevicePtr device_ptr, const EPSamplerDesc* desc, EPSamplerPtr* out_sampler) {
    ...
    auto [result, vk_sampler] = device->device->createSamplerUnique(::vk::SamplerCreateInfo{
        ...
        .mipmapMode = desc->min_filter == EP_FILTER_LINEAR
        ...
        .compareEnable = desc->compare_op != EP_COMPARE_ALWAYS,
    ...
    *out_sampler = reinterpret_cast<EPSamplerPtr>(sampler.release());
    return ok();
}
...
extern "C" EPError EPShaderLibraryCreate(EPDevicePtr device_ptr, const EPShaderLibraryDesc* desc,
                                          EPShaderLibraryPtr* out_library) {
    if (!device_ptr || !desc || !out_library) return invalid_argument("device, desc, or out_library is NULL");
    if (desc->format != EP_SHADER_SPIRV) return unsupported("only SPIR-V shaders supported on Vulkan");
    ...
    *out_library = reinterpret_cast<EPShaderLibraryPtr>(library.release());
    return ok();
}
