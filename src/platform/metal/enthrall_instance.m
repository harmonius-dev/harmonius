// enthrall_instance.m - Platform, instance, and adapter implementation
...
EPError EPPlatformCreate(EPPlatformPtr *out_platform) {
    ...
    EPPlatform *platform = calloc(1, sizeof(EPPlatform));
...
EPError EPInstanceCreate(const EPInstanceDesc *desc, EPInstancePtr *out_instance) {
    ...
    @autoreleasepool {
        EPInstance *instance = calloc(1, sizeof(EPInstance));
        ...
        if (desc->enable_backends & EP_BACKEND_METAL_BIT) {
...
EPError EPInstanceEnumerateAdapters(EPInstancePtr instance, uint32_t *io_count,
    ...
    @autoreleasepool {
        ...
        for (uint32_t i = 0; i < count; i++) {
            EPAdapter *adapter = calloc(1, sizeof(EPAdapter));
            ...
            adapter->properties.backends = EP_BACKEND_METAL_BIT;
...
EPError EPAdapterGetProperties(EPAdapterPtr adapter, EPAdapterProperties *out_properties) {
