// enthrall_sync.m - Fence and timeline semaphore implementation

#include "enthrall_internal.h"

#pragma mark - Fence

EPError EPFenceCreate(EPDevicePtr device, uint64_t initial_value, EPFencePtr *out_fence) {
    if (!device || !out_fence) return ep_invalid_argument("device or out_fence is NULL");

    @autoreleasepool {
        EPFence *fence = calloc(1, sizeof(EPFence));
        ...
        if (!fence->event) {
            free(fence);
            return ep_error(EP_E_DEVICE_LOST, "failed to create shared event");
        }
...
EPError EPTimelineSemaphoreCreate(EPDevicePtr device, uint64_t initial_value,
    ...
    @autoreleasepool {
        EPTimelineSemaphore *semaphore = calloc(1, sizeof(EPTimelineSemaphore));
...
EPError EPTimelineSemaphoreGetValue(EPTimelineSemaphorePtr semaphore, uint64_t *out_value) {
    if (!semaphore || !out_value) return ep_invalid_argument("semaphore or out_value is NULL");
    *out_value = semaphore->event.signaledValue;
    return ep_ok();
}

EPError EPTimelineSemaphoreWait(EPTimelineSemaphorePtr semaphore, uint64_t value, uint64_t timeout_ns) {
...
EPError EPTimelineSemaphoreSignal(EPTimelineSemaphorePtr semaphore, uint64_t value) {
    if (!semaphore) return ep_invalid_argument("semaphore is NULL");
