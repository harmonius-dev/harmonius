// gcraft_pipeline.m - Pipeline and acceleration structure implementation
...
GCError GCRenderPipelineCreate(GCDevicePtr device, const GCRenderPipelineDesc *desc,
    ...
    @autoreleasepool {
        GCRenderPipeline *pipeline = calloc(1, sizeof(GCRenderPipeline));
...
GCError GCComputePipelineCreate(GCDevicePtr device, const GCComputePipelineDesc *desc,
    ...
    @autoreleasepool {
        GCComputePipeline *pipeline = calloc(1, sizeof(GCComputePipeline));
        if (!pipeline) return ep_out_of_memory();
...
GCError GCMeshPipelineCreate(GCDevicePtr device, const GCMeshPipelineDesc *desc,
    ...
    @autoreleasepool {
        GCMeshPipeline *pipeline = calloc(1, sizeof(GCMeshPipeline));
        if (!pipeline) return ep_out_of_memory();
...
GCError GCRayTracingPipelineCreate(GCDevicePtr device, const GCRayTracingPipelineDesc *desc,
    ...
    @autoreleasepool {
        GCRayTracingPipeline *pipeline = calloc(1, sizeof(GCRayTracingPipeline));
        if (!pipeline) return ep_out_of_memory();
...
GCError GCAccelerationStructureCreate(GCDevicePtr device, const GCAccelerationStructureDesc *desc,
    ...
    @autoreleasepool {
        GCAccelerationStructure *as = calloc(1, sizeof(GCAccelerationStructure));
