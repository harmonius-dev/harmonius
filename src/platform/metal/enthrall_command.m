// enthrall_command.m - Command buffer and encoding implementation

#include "enthrall_internal.h"

#pragma mark - Command Buffer

EPError EPCommandBufferCreate(EPDevicePtr device, EPCommandBufferPtr *out_command_buffer) {
    if (!device || !out_command_buffer) return ep_invalid_argument("device or out_command_buffer is NULL");

    EPCommandBuffer *cmd = calloc(1, sizeof(EPCommandBuffer));
...
EPError EPCommandBufferBegin(EPCommandBufferPtr command_buffer) {
    ...
    if (command_buffer->is_recording) return ep_error(EP_E_INVALID_STATE, "already recording");
    ...
    @autoreleasepool {
        ...
        if (!command_buffer->buffer) {
            return ep_error(EP_E_DEVICE_LOST, "failed to create command buffer");
        }
...
EPError EPCommandBufferEnd(EPCommandBufferPtr command_buffer) {
    if (!command_buffer) return ep_invalid_argument("command_buffer is NULL");
    if (!command_buffer->is_recording) return ep_error(EP_E_INVALID_STATE, "not recording");
...
EPError EPCommandBeginRenderPass(EPCommandBufferPtr command_buffer, const EPRenderPassDesc *desc) {
    ...
    @autoreleasepool {
        ...
        if (!command_buffer->render_encoder) {
            return ep_error(EP_E_DEVICE_LOST, "failed to create render encoder");
        }
    }
...
EPError EPCommandBindRenderPipeline(EPCommandBufferPtr command_buffer, EPRenderPipelinePtr pipeline) {
    if (!command_buffer || !pipeline) return ep_invalid_argument("command_buffer or pipeline is NULL");
    if (!command_buffer->render_encoder) return ep_error(EP_E_INVALID_STATE, "no active render pass");
...
EPError EPCommandBindMeshPipeline(EPCommandBufferPtr command_buffer, EPMeshPipelinePtr pipeline) {
    if (!command_buffer || !pipeline) return ep_invalid_argument("command_buffer or pipeline is NULL");
    if (!command_buffer->render_encoder) return ep_error(EP_E_INVALID_STATE, "no active render pass");
...
EPError EPCommandBindComputePipeline(EPCommandBufferPtr command_buffer, EPComputePipelinePtr pipeline) {
    if (!command_buffer || !pipeline) return ep_invalid_argument("command_buffer or pipeline is NULL");
...
EPError EPCommandBindRayTracingPipeline(EPCommandBufferPtr command_buffer, EPRayTracingPipelinePtr pipeline) {
    if (!command_buffer || !pipeline) return ep_invalid_argument("command_buffer or pipeline is NULL");
...
static void ep_bind_descriptors_to_render(id<MTLRenderCommandEncoder> encoder,
    ...
    for (uint32_t i = 0; i < set->entry_count; i++) {
        ...
        EPDescriptorSetEntry *entry = &set->entries[i];
...
EPError EPCommandBindDescriptorSet(EPCommandBufferPtr command_buffer, EPPipelineLayoutPtr layout,
...
EPError EPCommandPushConstants(EPCommandBufferPtr command_buffer, EPPipelineLayoutPtr layout,
    ...
    @autoreleasepool {
        ...
        if (command_buffer->render_encoder) {
            if (stages & (EP_STAGE_VERTEX_BIT | EP_STAGE_MESH_BIT | EP_STAGE_TASK_BIT)) {
                [command_buffer->render_encoder setVertexBytes:data length:size atIndex:30];
            }
            if (stages & EP_STAGE_FRAGMENT_BIT) {
...
EPError EPCommandSetViewport(EPCommandBufferPtr command_buffer, float x, float y,
...
EPError EPCommandSetScissor(EPCommandBufferPtr command_buffer, uint32_t x, uint32_t y,
...
EPError EPCommandDraw(EPCommandBufferPtr command_buffer, uint32_t vertex_count,
...
EPError EPCommandDrawIndexed(EPCommandBufferPtr command_buffer, uint32_t index_count,
    ...
    return ep_unsupported("use EPCommandDraw or bind index buffer separately");
}

EPError EPCommandDispatch(EPCommandBufferPtr command_buffer, uint32_t group_x,
                          uint32_t group_y, uint32_t group_z) {
    if (!command_buffer) return ep_invalid_argument("command_buffer is NULL");
    if (!command_buffer->compute_encoder) return ep_error(EP_E_INVALID_STATE, "no active compute encoder");
...
EPError EPCommandDispatchMesh(EPCommandBufferPtr command_buffer, uint32_t group_x,
                              uint32_t group_y, uint32_t group_z) {
    if (!command_buffer) return ep_invalid_argument("command_buffer is NULL");
    if (!command_buffer->render_encoder) return ep_error(EP_E_INVALID_STATE, "no active render pass");
...
EPError EPCommandDispatchRays(EPCommandBufferPtr command_buffer, uint32_t width,
...
EPError EPCommandResourceBarrier(EPCommandBufferPtr command_buffer, const EPBarrierDesc *desc) {
    if (!command_buffer || !desc) return ep_invalid_argument("command_buffer or desc is NULL");
