...
GCError GCCommandBufferCreate(GCDevicePtr device,
  ...
  GCCommandBuffer *cmd =
      static_cast<GCCommandBuffer *>(calloc(1, sizeof(GCCommandBuffer)));
  if (!cmd)
...
GCError GCCommandBufferBegin(GCCommandBufferPtr command_buffer) {
  if (!command_buffer)
    return ep_invalid_argument("command_buffer is NULL");
  GCCommandBuffer *cmd = static_cast<GCCommandBuffer *>(command_buffer);
  if (cmd->is_recording)
...
GCError GCCommandBeginRenderPass(GCCommandBufferPtr command_buffer,
  ...
  for (uint32_t i = 0; i < desc->color_attachment_count; i++) {
    ...
    if (!att->texture)
      continue;
    GCTexture *tex = static_cast<GCTexture *>(
        const_cast<void *>(static_cast<const void *>(att->texture)));
    pass_desc->colorAttachments()->object(i)->setTexture(tex->texture);
    pass_desc->colorAttachments()->object(i)->setLoadAction(
        ep_to_mtl_load_action(att->load_op));
    pass_desc->colorAttachments()->object(i)->setStoreAction(
        ep_to_mtl_store_action(att->store_op));
    pass_desc->colorAttachments()->object(i)->setClearColor(
        MTL::ClearColor(att->clear_color[0], att->clear_color[1],
                        att->clear_color[2], att->clear_color[3]));
  }

  if (desc->depth_attachment && desc->depth_attachment->texture) {
    GCTexture *tex = static_cast<GCTexture *>(const_cast<void *>(
        static_cast<const void *>(desc->depth_attachment->texture)));
    pass_desc->depthAttachment()->setTexture(tex->texture);
    pass_desc->depthAttachment()->setLoadAction(
        ep_to_mtl_load_action(desc->depth_attachment->load_op));
    pass_desc->depthAttachment()->setStoreAction(
        ep_to_mtl_store_action(desc->depth_attachment->store_op));
    pass_desc->depthAttachment()->setClearDepth(
...
GCError GCCommandBindRenderPipeline(GCCommandBufferPtr command_buffer,
  ...
  GCCommandBuffer *cmd = static_cast<GCCommandBuffer *>(command_buffer);
  if (!cmd->render_encoder)
    return ep_error(GC_E_INVALID_STATE, "no active render pass");
...
GCError GCCommandBindRayTracingPipeline(GCCommandBufferPtr command_buffer,
  ...
  if (pipe->states && pipe->states->count() > 0) {
    cmd->compute_encoder->setComputePipelineState(
        static_cast<MTL::ComputePipelineState *>(pipe->states->object(0)));
  }
...
GCError GCCommandPushConstants(GCCommandBufferPtr command_buffer,
                               GCPipelineLayoutPtr layout,
                               GCShaderStageFlags stages, const uint8_t *data,
                               uint32_t size) {
  ...
  if (cmd->render_encoder) {
    if (stages &
        (GC_STAGE_VERTEX_BIT | GC_STAGE_MESH_BIT | GC_STAGE_TASK_BIT)) {
...
GCError GCCommandSetViewport(GCCommandBufferPtr command_buffer, float x,
...
GCError GCCommandSetScissor(GCCommandBufferPtr command_buffer, uint32_t x,
...
GCError GCCommandDraw(GCCommandBufferPtr command_buffer, uint32_t vertex_count,
  ...
  cmd->render_encoder->drawPrimitives(
      MTL::PrimitiveTypeTriangle, NS::UInteger(first_vertex),
      NS::UInteger(vertex_count), NS::UInteger(instance_count),
      NS::UInteger(first_instance));
  return ep_ok();
}
...
GCError GCCommandDispatch(GCCommandBufferPtr command_buffer, uint32_t group_x,
  ...
  GCCommandBuffer *cmd = static_cast<GCCommandBuffer *>(command_buffer);
  if (!cmd->compute_encoder)
    return ep_error(GC_E_INVALID_STATE, "no active compute encoder");
...
GCError GCCommandDispatchMesh(GCCommandBufferPtr command_buffer,
  ...
  cmd->render_encoder->drawMeshThreadgroups(threadgroups, threads_per_object,
                                            threads_per_mesh);
  return ep_ok();
}
...
GCError GCCommandResourceBarrier(GCCommandBufferPtr command_buffer,
  ...
  if (cmd->render_encoder) {
    ...
    cmd->compute_encoder->memoryBarrier(MTL::BarrierScopeBuffers |
  ...
  for (uint32_t i = 0; i < set->entry_count; i++) {
    ...
    case GC_DESCRIPTOR_STORAGE_BUFFER:
      encoder->setVertexBuffer(entry->value.buffer_info.buffer,
                               entry->value.buffer_info.offset, binding);
