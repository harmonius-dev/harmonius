// gcraft_resource.cpp - Buffer, texture, sampler, and shader library
// (Metal-cpp)
...
GCError GCBufferMap(GCBufferPtr buffer, void **out_data) {
  if (!buffer || !out_data)
    return ep_invalid_argument("buffer or out_data is NULL");
  GCBuffer *buf = static_cast<GCBuffer *>(buffer);
  if (!buf->host_visible)
    return ep_unsupported("buffer is not host-visible");
...
GCError GCBufferUnmap(GCBufferPtr buffer) {
  if (!buffer)
    return ep_invalid_argument("buffer is NULL");
  (void)buffer;
  return ep_ok();
}
...
GCError GCTextureCreate(GCDevicePtr device, const GCTextureDesc *desc,
  ...
  mtl_desc->setTextureType(
      ep_to_mtl_texture_type(desc->dimension, desc->array_layers));
...
GCError GCShaderLibraryCreate(GCDevicePtr device,
  ...
  if (desc->format != GC_SHADER_MSL)
    return ep_unsupported("only MSL shaders supported on Metal");

  GCDevice *dev = static_cast<GCDevice *>(device);
  GCShaderLibrary *library =
      static_cast<GCShaderLibrary *>(calloc(1, sizeof(GCShaderLibrary)));
  if (!library)
    return ep_out_of_memory();

  library->ep_device = dev;

  std::string src(reinterpret_cast<const char *>(desc->data),
  ...
  if (desc->label && dev->debug_names_enabled) {
    library->library->setLabel(
        NS::String::string(desc->label, NS::UTF8StringEncoding));
  }
