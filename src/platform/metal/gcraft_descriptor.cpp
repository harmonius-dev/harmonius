// gcraft_descriptor.cpp - Descriptor set layout, descriptor set, and pipeline
// layout (Metal-cpp)
...
GCError GCDescriptorSetLayoutCreate(GCDevicePtr device,
  ...
  GCDescriptorSetLayout *layout = static_cast<GCDescriptorSetLayout *>(
      calloc(1, sizeof(GCDescriptorSetLayout)));
  if (!layout)
  ...
  if (desc->binding_count > 0) {
    layout->bindings = static_cast<GCDescriptorBindingInfo *>(
        calloc(desc->binding_count, sizeof(GCDescriptorBindingInfo)));
    if (!layout->bindings) {
      free(layout);
      return ep_out_of_memory();
    }
...
GCError GCDescriptorSetCreate(GCDevicePtr device,
  ...
  GCDescriptorSetLayout *lay = static_cast<GCDescriptorSetLayout *>(layout);
  GCDescriptorSet *set =
      static_cast<GCDescriptorSet *>(calloc(1, sizeof(GCDescriptorSet)));
  if (!set)
    return ep_out_of_memory();
  ...
  if (lay->binding_count > 0) {
    set->entries = static_cast<GCDescriptorSetEntry *>(
        calloc(lay->binding_count, sizeof(GCDescriptorSetEntry)));
    if (!set->entries) {
      free(set);
      return ep_out_of_memory();
    }
...
GCError GCDescriptorSetUpdateBuffer(GCDescriptorSetPtr set, uint32_t binding,
                                    ...
                                    uint64_t range) {
  if (!set || !buffer)
    return ep_invalid_argument("set or buffer is NULL");
  GCDescriptorSet *s = static_cast<GCDescriptorSet *>(set);
...
GCError GCDescriptorSetUpdateTexture(GCDescriptorSetPtr set, uint32_t binding,
  ...
  if (!set || !texture)
    return ep_invalid_argument("set or texture is NULL");
  GCDescriptorSet *s = static_cast<GCDescriptorSet *>(set);
...
GCError GCDescriptorSetUpdateSampler(GCDescriptorSetPtr set, uint32_t binding,
  ...
  if (!set || !sampler)
    return ep_invalid_argument("set or sampler is NULL");
  GCDescriptorSet *s = static_cast<GCDescriptorSet *>(set);
...
GCError GCDescriptorSetUpdateAccelerationStructure(
    GCDescriptorSetPtr set, uint32_t binding, GCAccelerationStructurePtr as) {
  if (!set || !as)
    return ep_invalid_argument("set or acceleration structure is NULL");
  GCDescriptorSet *s = static_cast<GCDescriptorSet *>(set);
...
GCError GCPipelineLayoutCreate(GCDevicePtr device,
  ...
  GCPipelineLayout *layout =
      static_cast<GCPipelineLayout *>(calloc(1, sizeof(GCPipelineLayout)));
  if (!layout)
    return ep_out_of_memory();
  ...
  if (desc->set_layout_count > 0) {
    layout->set_layouts = static_cast<GCDescriptorSetLayout **>(
        calloc(desc->set_layout_count, sizeof(GCDescriptorSetLayout *)));
    ...
    for (uint32_t i = 0; i < desc->set_layout_count; i++) {
      layout->set_layouts[i] = static_cast<GCDescriptorSetLayout *>(
          const_cast<void *>(static_cast<const void *>(desc->set_layouts[i])));
    }
  }
