  ...
  do {                                                                         \
    if ((err).code != GC_E_OK) {                                               \
      fprintf(stderr, "FAIL: %s (line %d): error %d: %s\n", __func__,          \
              __LINE__, (err).code, (err).message ? (err).message : "null");   \
      return 1;                                                                \
    }                                                                          \
  } while (0)

#define TEST_ASSERT_ERR(err, expected_code)                                    \
  do {                                                                         \
...
static int setup_test_context(TestContext *ctx) {
  ...
  if (err.code != GC_E_OK || adapter_count == 0)
    return -1;

  GCAdapterPtr *adapters =
      static_cast<GCAdapterPtr *>(malloc(adapter_count * sizeof(GCAdapterPtr)));
  ...
  // Get queues
  err =
      GCDeviceGetQueue(ctx->device, GC_QUEUE_GRAPHICS, 0, &ctx->graphics_queue);
  if (err.code != GC_E_OK)
    return -1;
...
static void teardown_test_context(TestContext *ctx) {
  if (ctx->device)
    GCDeviceDestroy(ctx->device);
  if (ctx->instance)
    GCInstanceDestroy(ctx->instance);
}
...
static int test_instance_enumerate_adapters(void) {
  ...
  GCAdapterPtr *adapters =
      static_cast<GCAdapterPtr *>(malloc(count * sizeof(GCAdapterPtr)));
...
static int test_adapter_properties(void) {
  ...
  TEST_ASSERT(props.backends & GC_BACKEND_METAL_BIT,
              "should report Metal support");
  TEST_ASSERT(props.features & GC_FEATURE_COMPUTE_BIT,
              "should support compute");
  TEST_ASSERT(props.limits.max_texture_dimension_2d > 0,
              "max texture 2D should be > 0");
  TEST_ASSERT(props.limits.max_threads_per_threadgroup > 0,
              "max threads should be > 0");
...
static int test_buffer_create_host_visible(void) {
  ...
  GCBufferDesc desc = {
      .size = 1024,
      .usage = static_cast<GCBufferUsageFlags>(GC_BUFFER_USAGE_UNIFORM_BIT |
...
static int test_buffer_create_device_local(void) {
  ...
  GCBufferDesc desc = {
      .size = 4096,
      .usage = static_cast<GCBufferUsageFlags>(
          GC_BUFFER_USAGE_STORAGE_BIT | GC_BUFFER_USAGE_TRANSFER_DST_BIT),
      .host_visible = false,
  };
...
static int test_texture_create_2d(void) {
  ...
  GCTextureDesc desc = {
      ...
      .mip_levels = 1,
      .array_layers = 1,
      .usage = static_cast<GCTextureUsageFlags>(GC_TEXTURE_USAGE_SAMPLED_BIT |
                                                GC_TEXTURE_USAGE_STORAGE_BIT),
  };
...
static int test_texture_create_depth(void) {
  ...
  GCTextureDesc desc = {
      ...
      .mip_levels = 1,
      .array_layers = 1,
      .usage = static_cast<GCTextureUsageFlags>(
          GC_TEXTURE_USAGE_DEPTH_ATTACHMENT_BIT | GC_TEXTURE_USAGE_SAMPLED_BIT),
  };
...
static int test_descriptor_set_layout_create(void) {
  ...
  GCDescriptorSetLayoutDesc *desc = static_cast<GCDescriptorSetLayoutDesc *>(
      malloc(sizeof(GCDescriptorSetLayoutDesc) +
             1 * sizeof(GCDescriptorBindingDesc)));
...
static int test_descriptor_set_create_and_update(void) {
  ...
  // Create layout
  GCDescriptorSetLayoutDesc *layout_desc =
      static_cast<GCDescriptorSetLayoutDesc *>(
          malloc(sizeof(GCDescriptorSetLayoutDesc) +
                 1 * sizeof(GCDescriptorBindingDesc)));
...
static int test_compute_pipeline_create(void) {
  ...
  // Create pipeline layout
  GCPipelineLayoutDesc *layout_desc =
      static_cast<GCPipelineLayoutDesc *>(malloc(sizeof(GCPipelineLayoutDesc)));
...
static int test_compute_execution_verify_output(void) {
  ...
  // Create pipeline layout
  GCPipelineLayoutDesc *pipe_layout_desc = static_cast<GCPipelineLayoutDesc *>(
      malloc(sizeof(GCPipelineLayoutDesc) + sizeof(GCDescriptorSetLayoutPtr)));
...
static int test_compute_with_push_constants(void) {
  ...
  // Create descriptor set layout
  GCDescriptorSetLayoutDesc *layout_desc =
      static_cast<GCDescriptorSetLayoutDesc *>(malloc(
  ...
  for (uint32_t i = 0; i < element_count; i++) {
    ...
    if (fabsf(data2[i] - expected) > 0.0001f) {
      printf("\n  Mismatch at [%u]: expected %f, got %f", i, expected,
             data2[i]);
      all_correct = false;
    }
  }
...
static int test_render_pass_begin_end(void) {
  ...
  GCRenderPassDesc *pass_desc = static_cast<GCRenderPassDesc *>(
      malloc(sizeof(GCRenderPassDesc) + sizeof(GCRenderPassColorAttachment)));
