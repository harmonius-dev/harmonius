...
GCError GCDeviceCreate(GCAdapterPtr adapter, const GCDeviceDesc *desc,
  ...
  if ((desc->required_features & adapter->properties.features) !=
  ...
  if (!device->graphics_queue || !device->compute_queue ||
    ...
    if (device->graphics_queue)
      device->graphics_queue->release();
    if (device->compute_queue)
      device->compute_queue->release();
    if (device->transfer_queue)
      device->transfer_queue->release();
  ...
  if (desc->enable_debug_names) {
    device->graphics_queue->setLabel(
        NS::String::string("Gcraft Graphics Queue", NS::UTF8StringEncoding));
    device->compute_queue->setLabel(
        NS::String::string("Gcraft Compute Queue", NS::UTF8StringEncoding));
    device->transfer_queue->setLabel(
        NS::String::string("Gcraft Transfer Queue", NS::UTF8StringEncoding));
  }
...
GCError GCDeviceGetQueue(GCDevicePtr device, GCQueueType type, uint32_t index,
  ...
  if (!device || !out_queue)
    return ep_invalid_argument("device or out_queue is NULL");
  if (index > 0)
    return ep_invalid_argument("only queue index 0 is supported");
...
GCError GCQueueSubmit(GCQueuePtr queue, const GCSubmitInfo *submit) {
  if (!queue || !submit)
    return ep_invalid_argument("queue or submit is NULL");

  for (uint32_t i = 0; i < submit->command_buffer_count; i++) {
    GCCommandBuffer *cmd = static_cast<GCCommandBuffer *>(const_cast<void *>(
        static_cast<const void *>(submit->command_buffers[i])));
    if (!cmd || !cmd->buffer)
      continue;

    for (uint32_t w = 0; w < submit->wait_count; w++) {
      GCTimelineSemaphore *sem =
          static_cast<GCTimelineSemaphore *>(const_cast<void *>(
              static_cast<const void *>(submit->wait_semaphores[w])));
      if (sem && sem->event) {
    ...
    for (uint32_t s = 0; s < submit->signal_count; s++) {
      GCTimelineSemaphore *sem =
          static_cast<GCTimelineSemaphore *>(const_cast<void *>(
              static_cast<const void *>(submit->signal_semaphores[s])));
      if (sem && sem->event) {
...
GCError GCQueueWaitIdle(GCQueuePtr queue) {
  if (!queue)
    return ep_invalid_argument("queue is NULL");
