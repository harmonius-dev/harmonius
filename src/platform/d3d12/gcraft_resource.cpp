...
extern "C" GCError GCBufferCreate(GCDevicePtr device, const GCBufferDesc *desc,
                                  GCBufferPtr *out_buffer) {
    if (!device || !desc || !out_buffer)
        return ep_invalid_argument("device, desc, or out_buffer is NULL");
    
    GCBuffer *buffer = new (std::nothrow) GCBuffer{};
    ...
    if (desc->usage & GC_BUFFER_USAGE_STORAGE_BIT) {
...
extern "C" GCError GCBufferDestroy(GCBufferPtr buffer) {
    if (buffer) {
...
extern "C" GCError GCTextureCreate(GCDevicePtr device, const GCTextureDesc *desc,
                                   GCTexturePtr *out_texture) {
    if (!device || !desc || !out_texture)
        return ep_invalid_argument("device, desc, or out_texture is NULL");
    
    GCTexture *texture = new (std::nothrow) GCTexture{};
    ...
    if (desc->dimension == GC_TEXTURE_DIM_3D) {
        depth_or_array_size = static_cast<UINT16>(desc->depth);
    } else if (desc->dimension == GC_TEXTURE_DIM_CUBE) {
    ...
    if (desc->usage & GC_TEXTURE_USAGE_COLOR_ATTACHMENT_BIT) {
        resource_desc.Flags |= D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET;
    }
    if (desc->usage & GC_TEXTURE_USAGE_DEPTH_ATTACHMENT_BIT) {
        resource_desc.Flags |= D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL;
    }
    if (desc->usage & GC_TEXTURE_USAGE_STORAGE_BIT) {
        resource_desc.Flags |= D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
    }
    ...
    } else if (desc->usage & GC_TEXTURE_USAGE_DEPTH_ATTACHMENT_BIT) {
        clear_value.Format = resource_desc.Format;
    ...
    if (desc->usage & GC_TEXTURE_USAGE_DEPTH_ATTACHMENT_BIT) {
        ...
        switch (desc->dimension) {
            ...
            case GC_TEXTURE_DIM_2D:
            case GC_TEXTURE_DIM_CUBE:
            case GC_TEXTURE_DIM_3D:  // 3D depth not typically supported, but handle as 2D array
                if (resource_desc.DepthOrArraySize > 1) {
    ...
    if (desc->usage & GC_TEXTURE_USAGE_SAMPLED_BIT) {
    ...
    if (desc->usage & GC_TEXTURE_USAGE_STORAGE_BIT) {
        ...
        switch (desc->dimension) {
            case GC_TEXTURE_DIM_1D:
                ...
                break;
            case GC_TEXTURE_DIM_2D:
            case GC_TEXTURE_DIM_CUBE:  // Cube UAV treated as 2D array
                if (resource_desc.DepthOrArraySize > 1) {
...
extern "C" GCError GCTextureDestroy(GCTexturePtr texture) {
    delete texture;
    return ep_ok();
}
...
extern "C" GCError GCSamplerCreate(GCDevicePtr device, const GCSamplerDesc *desc,
                                   GCSamplerPtr *out_sampler) {
    if (!device || !desc || !out_sampler)
        return ep_invalid_argument("device, desc, or out_sampler is NULL");
    
    GCSampler *sampler = new (std::nothrow) GCSampler{};
...
extern "C" GCError GCShaderLibraryCreate(GCDevicePtr device, const GCShaderLibraryDesc *desc,
    ...
    if (desc->size > 4 && memcmp(desc->data, "DXBC", 4) == 0) {
        ...
        if (FAILED(hr)) {
            ...
            delete library;
            return ep_error(GC_E_INVALID_ARGUMENT, "shader compilation failed");
        }
