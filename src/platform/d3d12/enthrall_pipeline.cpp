...
static DXGI_FORMAT ep_to_vertex_format(EPTextureFormat format) {
    switch (format) {
...
extern "C" EPError EPRenderPipelineCreate(EPDevicePtr device,
    ...
    EPRenderPipeline *pipeline = new (std::nothrow) EPRenderPipeline{};
...
extern "C" EPError EPComputePipelineCreate(EPDevicePtr device,
    ...
    EPComputePipeline *pipeline = new (std::nothrow) EPComputePipeline{};
    if (!pipeline) return ep_out_of_memory();
...
extern "C" EPError EPMeshPipelineCreate(EPDevicePtr device,
    ...
    EPMeshPipeline *pipeline = new (std::nothrow) EPMeshPipeline{};
    if (!pipeline) return ep_out_of_memory();
...
extern "C" EPError EPRayTracingPipelineCreate(EPDevicePtr device,
    ...
    EPRayTracingPipeline *pipeline = new (std::nothrow) EPRayTracingPipeline{};
    if (!pipeline) return ep_out_of_memory();
...
extern "C" EPError EPAccelerationStructureCreate(EPDevicePtr device,
    ...
    EPAccelerationStructure *as = new (std::nothrow) EPAccelerationStructure{};
...
extern "C" EPError EPAccelerationStructureDestroy(EPAccelerationStructurePtr as) {
    delete as;
    return ep_ok();
}
