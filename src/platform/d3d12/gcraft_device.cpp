...
static GCError create_descriptor_heaps(GCDevice *device) {
...
extern "C" GCError GCDeviceCreate(GCAdapterPtr adapter, const GCDeviceDesc *desc,
    ...
    GCDevice *device = new (std::nothrow) GCDevice{};
    ...
    if (desc->enable_debug_names) {
        device->device->SetName(L"Gcraft Device");
    }
    ...
    if (desc->enable_debug_names) {
        device->graphics_queue->SetName(L"Gcraft Graphics Queue");
        device->compute_queue->SetName(L"Gcraft Compute Queue");
        device->copy_queue->SetName(L"Gcraft Copy Queue");
    }
    
    // Create descriptor heaps
    GCError err = create_descriptor_heaps(device);
    if (err.code != GC_E_OK) {
        delete device;
        return err;
    }
...
extern "C" GCError GCDeviceDestroy(GCDevicePtr device) {
    delete device;
    return ep_ok();
}

extern "C" GCError GCDeviceGetQueue(GCDevicePtr device, GCQueueType type, uint32_t index,
    ...
    GCQueue *queue = new (std::nothrow) GCQueue{};
...
extern "C" GCError GCQueueSubmit(GCQueuePtr queue, const GCSubmitInfo *submit) {
...
extern "C" GCError GCQueueWaitIdle(GCQueuePtr queue) {
    if (!queue) return ep_invalid_argument("queue is NULL");
    
    GCDevice *device = queue->ep_device;
