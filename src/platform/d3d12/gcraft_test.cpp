// gcraft_test.cpp - Meaningful integration tests for D3D12 backend
...
#define TEST_OK(err) TEST_ASSERT((err).code == GC_E_OK, (err).message ? (err).message : "unexpected error")
...
static GCBufferPtr create_readback_buffer(uint64_t size) {
    ...
    GCBufferDesc desc = {};
    desc.size = size;
    desc.usage = GC_BUFFER_USAGE_TRANSFER_DST_BIT;
    desc.host_visible = true;
    
    GCBufferPtr buffer = nullptr;
    GCError err = GCBufferCreate(g_device, &desc, &buffer);
    if (err.code != GC_E_OK) return nullptr;
    return buffer;
}

// Helper: Create a staging/upload buffer with initial data
static GCBufferPtr create_staging_buffer(const void *data, uint64_t size) {
    GCBufferDesc desc = {};
    desc.size = size;
    desc.usage = GC_BUFFER_USAGE_TRANSFER_SRC_BIT;
    desc.host_visible = true;
...
static GCBufferPtr create_gpu_buffer(uint64_t size, GCBufferUsageFlags usage) {
    GCBufferDesc desc = {};
    desc.size = size;
    desc.usage = usage;
...
static bool submit_and_wait(GCCommandBufferPtr cmd) {
    GCQueuePtr queue = nullptr;
    GCError err = GCDeviceGetQueue(g_device, GC_QUEUE_GRAPHICS, 0, &queue);
    if (err.code != GC_E_OK) return false;
    ...
    err = GCQueueSubmit(queue, &submit);
    if (err.code != GC_E_OK) {
        delete queue;
        return false;
    }
    
    err = GCQueueWaitIdle(queue);
    delete queue;
    return err.code == GC_E_OK;
}

// Helper: Copy GPU buffer to CPU-readable buffer and return contents
static std::vector<uint8_t> readback_buffer(GCBufferPtr gpu_buffer, uint64_t size) {
    std::vector<uint8_t> result;
    
    // Create readback buffer
    GCBufferPtr readback = create_readback_buffer(size);
    if (!readback) return result;
    
    // Create command buffer for copy
    GCCommandBufferPtr cmd = nullptr;
    GCError err = GCCommandBufferCreate(g_device, &cmd);
    if (err.code != GC_E_OK) {
        GCBufferDestroy(readback);
        return result;
    }
...
static bool upload_to_buffer(GCBufferPtr gpu_buffer, const void *data, uint64_t size) {
    // Create staging buffer with data
    GCBufferPtr staging = create_staging_buffer(data, size);
...
static bool setup_test_environment() {
    ...
    adapter_count = 1;
    err = GCInstanceEnumerateAdapters(g_instance, &adapter_count, &g_adapter);
    TEST_OK(err);
    TEST_ASSERT(g_adapter != nullptr, "adapter should not be null");
    
    // Create device
    GCDeviceDesc device_desc = {};
    device_desc.preferred_backend = GC_BACKEND_D3D12;
    device_desc.required_features = GC_FEATURE_COMPUTE_BIT;
    device_desc.optional_features = GC_FEATURE_MESH_SHADER_BIT | GC_FEATURE_RAY_TRACING_BIT;
    device_desc.enable_validation = true;
    device_desc.enable_debug_names = true;
    
    err = GCDeviceCreate(g_adapter, &device_desc, &g_device);
    TEST_OK(err);
...
static void teardown_test_environment() {
    if (g_device) GCDeviceDestroy(g_device);
    if (g_adapter) { /* adapter cleanup handled by instance */ }
    if (g_instance) GCInstanceDestroy(g_instance);
    if (g_platform) GCPlatformDestroy(g_platform);
...
static bool test_buffer_copy_roundtrip() {
    ...
    // Create GPU buffer
    GCBufferPtr gpu_buffer = create_gpu_buffer(buffer_size, 
        GC_BUFFER_USAGE_TRANSFER_SRC_BIT | GC_BUFFER_USAGE_TRANSFER_DST_BIT);
...
static bool test_buffer_partial_copy() {
    ...
    GCBufferPtr src_buffer = create_staging_buffer(test_data.data(), total_size);
    TEST_ASSERT(src_buffer != nullptr, "failed to create source buffer");
    
    // Create destination GPU buffer (zeroed)
    GCBufferPtr dst_buffer = create_gpu_buffer(total_size,
        GC_BUFFER_USAGE_TRANSFER_SRC_BIT | GC_BUFFER_USAGE_TRANSFER_DST_BIT);
    ...
    GCCommandBufferDestroy(cmd);
    GCBufferDestroy(src_buffer);
    GCBufferDestroy(dst_buffer);
    return true;
}
...
static bool test_compute_shader_write_pattern() {
    ...
    GCBufferDesc uav_desc = {};
    uav_desc.size = buffer_size;
    uav_desc.usage = GC_BUFFER_USAGE_STORAGE_BIT | GC_BUFFER_USAGE_TRANSFER_SRC_BIT;
    uav_desc.host_visible = false;
    
    GCBufferPtr output_buffer = nullptr;
    GCError err = GCBufferCreate(g_device, &uav_desc, &output_buffer);
    if (err.code != GC_E_OK) {
    ...
    shader_desc.format = GC_SHADER_HLSL;
    ...
    err = GCShaderLibraryCreate(g_device, &shader_desc, &shader);
    if (err.code != GC_E_OK) {
        GCBufferDestroy(output_buffer);
    ...
    GCCommandBufferPtr cmd = nullptr;
    err = GCCommandBufferCreate(g_device, &cmd);
    TEST_OK(err);
...
static bool test_render_pass_clear_color() {
    ...
    tex_desc.dimension = GC_TEXTURE_DIM_2D;
    tex_desc.format = GC_FORMAT_RGBA8_UNORM;
    tex_desc.width = width;
    tex_desc.height = height;
    tex_desc.depth = 1;
    tex_desc.mip_levels = 1;
    tex_desc.array_layers = 1;
    tex_desc.usage = GC_TEXTURE_USAGE_COLOR_ATTACHMENT_BIT | GC_TEXTURE_USAGE_TRANSFER_SRC_BIT;
    
    GCTexturePtr render_target = nullptr;
    GCError err = GCTextureCreate(g_device, &tex_desc, &render_target);
    if (err.code != GC_E_OK) {
    ...
    // Create readback buffer
    GCBufferPtr readback = create_readback_buffer(buffer_size);
    ...
    // Create render pass
    struct {
        GCRenderPassDesc header;
        GCRenderPassColorAttachment attachments[1];
    } rp_desc = {};
    rp_desc.header.color_attachment_count = 1;
    rp_desc.attachments[0].texture = render_target;
    rp_desc.attachments[0].layout = GC_TEXTURE_LAYOUT_COLOR_ATTACHMENT;
    rp_desc.attachments[0].load_op = GC_LOAD_OP_CLEAR;
    rp_desc.attachments[0].store_op = GC_STORE_OP_STORE;
    ...
    // Begin render pass
    err = GCCommandBeginRenderPass(cmd, &rp_desc.header);
    TEST_OK(err);
    ...
    GCCommandBufferDestroy(cmd);
    GCTextureDestroy(render_target);
    GCBufferDestroy(readback);
...
static bool test_render_pass_clear_depth() {
    ...
    GCTextureDesc tex_desc = {};
    tex_desc.dimension = GC_TEXTURE_DIM_2D;
    tex_desc.format = GC_FORMAT_D32_FLOAT;
    tex_desc.width = width;
    tex_desc.height = height;
    tex_desc.depth = 1;
    tex_desc.mip_levels = 1;
    tex_desc.array_layers = 1;
    tex_desc.usage = GC_TEXTURE_USAGE_DEPTH_ATTACHMENT_BIT | GC_TEXTURE_USAGE_TRANSFER_SRC_BIT;
    
    GCTexturePtr depth_texture = nullptr;
    GCError err = GCTextureCreate(g_device, &tex_desc, &depth_texture);
    if (err.code != GC_E_OK) {
    ...
    GCBufferPtr readback = create_readback_buffer(total_size);
    ...
    GCRenderPassDepthAttachment depth_att = {};
    depth_att.texture = depth_texture;
    depth_att.layout = GC_TEXTURE_LAYOUT_DEPTH_STENCIL;
    depth_att.load_op = GC_LOAD_OP_CLEAR;
    depth_att.store_op = GC_STORE_OP_STORE;
    depth_att.clear_depth = clear_depth;
    depth_att.clear_stencil = 0;
    
    GCRenderPassDesc rp_desc = {};
    ...
    err = GCCommandBeginRenderPass(cmd, &rp_desc);
    TEST_OK(err);
    ...
    GCCommandBufferDestroy(cmd);
    GCTextureDestroy(depth_texture);
    GCBufferDestroy(readback);
...
static bool test_fence_gpu_signal() {
    ...
    GCError err = GCFenceCreate(g_device, 0, &fence);
    TEST_OK(err);
    ...
    GCQueuePtr queue = nullptr;
    err = GCDeviceGetQueue(g_device, GC_QUEUE_GRAPHICS, 0, &queue);
    TEST_OK(err);
...
static bool test_timeline_semaphore_ordering() {
    ...
    GCError err = GCTimelineSemaphoreCreate(g_device, 0, &sem);
    TEST_OK(err);
    
    // Create two buffers
    const uint64_t buffer_size = 256;
    GCBufferPtr buffer_a = create_gpu_buffer(buffer_size,
        GC_BUFFER_USAGE_TRANSFER_SRC_BIT | GC_BUFFER_USAGE_TRANSFER_DST_BIT);
    GCBufferPtr buffer_b = create_gpu_buffer(buffer_size,
        GC_BUFFER_USAGE_TRANSFER_SRC_BIT | GC_BUFFER_USAGE_TRANSFER_DST_BIT);
    ...
    GCCommandBufferPtr cmd1 = nullptr;
    err = GCCommandBufferCreate(g_device, &cmd1);
    TEST_OK(err);
    
    err = GCCommandBufferBegin(cmd1);
    TEST_OK(err);
    
    cmd1->list->CopyBufferRegion(buffer_b->resource.Get(), 0,
                                  buffer_a->resource.Get(), 0, buffer_size);
    
    err = GCCommandBufferEnd(cmd1);
    TEST_OK(err);
    ...
    GCCommandBufferPtr cmds1[] = { cmd1 };
    uint64_t signal_val1 = 1;
    GCSubmitInfo submit1 = {};
    ...
    err = GCQueueSubmit(queue, &submit1);
    TEST_OK(err);
    
    // Wait on semaphore from CPU
    err = GCTimelineSemaphoreWait(sem, 1, 5000000000);
    TEST_OK(err);
    ...
    delete queue;
    GCCommandBufferDestroy(cmd1);
    GCTimelineSemaphoreDestroy(sem);
    GCBufferDestroy(buffer_a);
    GCBufferDestroy(buffer_b);
    return true;
}
...
static bool test_queue_wait_idle_actually_waits() {
    ...
    // Submit upload
    GCBufferPtr staging = create_staging_buffer(test_data.data(), buffer_size);
...
static bool test_dstorage_file_to_buffer() {
    ...
    GCBufferPtr gpu_buffer = create_gpu_buffer(file_size,
        GC_BUFFER_USAGE_TRANSFER_SRC_BIT | GC_BUFFER_USAGE_TRANSFER_DST_BIT);
    if (!gpu_buffer) {
        std::remove(test_filename);
...
static bool test_texture_upload_download_rgba8() {
    ...
    tex_desc.mip_levels = 1;
    tex_desc.array_layers = 1;
    tex_desc.usage = GC_TEXTURE_USAGE_SAMPLED_BIT | GC_TEXTURE_USAGE_TRANSFER_SRC_BIT | 
                     GC_TEXTURE_USAGE_TRANSFER_DST_BIT;
    
    GCTexturePtr texture = nullptr;
    GCError err = GCTextureCreate(g_device, &tex_desc, &texture);
    if (err.code != GC_E_OK) {
    ...
    GCBufferPtr staging = create_staging_buffer(aligned_data.data(), total_size);
    TEST_ASSERT(staging != nullptr, "failed to create staging buffer");
    ...
    GCCommandBufferPtr cmd2 = nullptr;
    err = GCCommandBufferCreate(g_device, &cmd2);
    TEST_OK(err);
    
    err = GCCommandBufferBegin(cmd2);
    TEST_OK(err);
    ...
    GCCommandBufferDestroy(cmd);
    GCCommandBufferDestroy(cmd2);
    GCTextureDestroy(texture);
    GCBufferDestroy(staging);
    GCBufferDestroy(readback);
...
static bool test_platform_create_destroy() {
    ...
    GCError err = GCPlatformCreate(&platform);
    TEST_OK(err);
    TEST_ASSERT(platform != nullptr, "platform should not be null");
    
    err = GCPlatformDestroy(platform);
    TEST_OK(err);
...
static bool test_instance_create_destroy() {
    ...
    GCInstanceDesc desc = {};
    desc.enable_backends = GC_BACKEND_D3D12_BIT;
    
    GCError err = GCInstanceCreate(&desc, &instance);
    TEST_OK(err);
...
static bool test_adapter_properties() {
    ...
    GCError err = GCAdapterGetProperties(g_adapter, &props);
    TEST_OK(err);
    
    TEST_ASSERT(strlen(props.name) > 0, "adapter name should not be empty");
    TEST_ASSERT(props.backends & GC_BACKEND_D3D12_BIT, "should support D3D12 backend");
...
static bool test_format_conversions() {
    TEST_ASSERT(ep_to_dxgi_format(GC_FORMAT_RGBA8_UNORM) == DXGI_FORMAT_R8G8B8A8_UNORM,
                "RGBA8 conversion");
    TEST_ASSERT(ep_to_dxgi_format(GC_FORMAT_BGRA8_UNORM) == DXGI_FORMAT_B8G8R8A8_UNORM,
                "BGRA8 conversion");
    TEST_ASSERT(ep_to_dxgi_format(GC_FORMAT_D32_FLOAT) == DXGI_FORMAT_D32_FLOAT,
                "D32 conversion");
    TEST_ASSERT(ep_to_d3d12_address(GC_ADDRESS_REPEAT) == D3D12_TEXTURE_ADDRESS_MODE_WRAP,
                "repeat address mode");
    TEST_ASSERT(ep_to_d3d12_compare(GC_COMPARE_LESS) == D3D12_COMPARISON_FUNC_LESS,
                "less compare");
...
int main() {
    printf("=== Gcraft D3D12 Backend Integration Tests ===\n\n");
