...
extern "C" GCError GCFenceCreate(GCDevicePtr device, uint64_t initial_value,
                                 GCFencePtr *out_fence) {
    if (!device || !out_fence) return ep_invalid_argument("device or out_fence is NULL");
    
    GCFence *fence = new (std::nothrow) GCFence{};
    ...
    if (!fence->event) {
        delete fence;
        return ep_error(GC_E_INVALID_STATE, "failed to create event");
    }
...
extern "C" GCError GCFenceWait(GCFencePtr fence, uint64_t value, uint64_t timeout_ns) {
    ...
    if (result == WAIT_TIMEOUT) {
        ...
        return ep_error(GC_E_INVALID_STATE, "fence wait failed");
    }
    
    return ep_ok();
}

extern "C" GCError GCFenceSignal(GCFencePtr fence, uint64_t value) {
    if (!fence) return ep_invalid_argument("fence is NULL");
...
extern "C" GCError GCTimelineSemaphoreCreate(GCDevicePtr device, uint64_t initial_value,
                                             GCTimelineSemaphorePtr *out_semaphore) {
    if (!device || !out_semaphore) 
        return ep_invalid_argument("device or out_semaphore is NULL");
    
    GCTimelineSemaphore *semaphore = new (std::nothrow) GCTimelineSemaphore{};
...
extern "C" GCError GCTimelineSemaphoreGetValue(GCTimelineSemaphorePtr semaphore,
                                               uint64_t *out_value) {
...
extern "C" GCError GCTimelineSemaphoreWait(GCTimelineSemaphorePtr semaphore,
    ...
    if (!event) return ep_error(GC_E_INVALID_STATE, "failed to create event");
    ...
    if (result == WAIT_TIMEOUT) {
        ...
        return ep_error(GC_E_INVALID_STATE, "semaphore wait failed");
    }
    
    return ep_ok();
}

extern "C" GCError GCTimelineSemaphoreSignal(GCTimelineSemaphorePtr semaphore,
                                             uint64_t value) {
    if (!semaphore) return ep_invalid_argument("semaphore is NULL");
...
extern "C" GCError GCSurfaceCreate(GCPlatformPtr platform, const GCSurfaceDesc *desc,
                                   GCSurfacePtr *out_surface) {
    if (!desc || !out_surface) return ep_invalid_argument("desc or out_surface is NULL");
    if (desc->handle_type != GC_NATIVE_HANDLE_HWND) 
        return ep_unsupported("only HWND surfaces supported on D3D12");
    
    GCSurface *surface = new (std::nothrow) GCSurface{};
...
extern "C" GCError GCSurfaceDestroy(GCSurfacePtr surface) {
    delete surface;
    return ep_ok();
}

extern "C" GCError GCSwapchainCreate(GCDevicePtr device, const GCSwapchainDesc *desc,
    ...
    GCSwapchain *swapchain = new (std::nothrow) GCSwapchain{};
...
extern "C" GCError GCSwapchainDestroy(GCSwapchainPtr swapchain) {
    delete swapchain;
    return ep_ok();
}

extern "C" GCError GCSwapchainAcquireNext(GCSwapchainPtr swapchain,
...
extern "C" GCError GCSwapchainPresent(GCSwapchainPtr swapchain,
