...
/// A pool that allocates and recycles Metal command buffers.
...
class MetalCommandPool {
 public:
    ...
    [[nodiscard]] auto AllocateCommandBuffer() -> MetalCommandBuffer;
    ...
    MetalCommandPool(const MetalCommandPool&) = delete;
    auto operator=(const MetalCommandPool&) -> MetalCommandPool& = delete;
    MetalCommandPool(MetalCommandPool&&) noexcept;
    auto operator=(MetalCommandPool&&) noexcept -> MetalCommandPool&;
    ~MetalCommandPool();
};
...
/// A Metal command buffer that records GPU commands for submission.
/// @threadsafety Instances are not thread-safe.
class MetalCommandBuffer {
 public:
    ...
    auto operator=(const MetalCommandBuffer&) -> MetalCommandBuffer& = delete;
    MetalCommandBuffer(MetalCommandBuffer&&) noexcept;
    auto operator=(MetalCommandBuffer&&) noexcept -> MetalCommandBuffer&;
    ~MetalCommandBuffer();
};
...
/// A Metal GPU device that manages resources, pipelines, and command submission.
/// @threadsafety Instances are not thread-safe.
class MetalDevice {
 public:
    using CommandBufferType = MetalCommandBuffer;
    ...
    [[nodiscard]] auto CreateBuffer(const BufferDesc& desc) -> std::expected<BufferHandle, ResourceError>;
    ...
    [[nodiscard]] auto CreateFence(std::uint64_t initial_value) -> std::expected<FenceHandle, DeviceError>;
    ...
    [[nodiscard]] auto CreateCommandPool(QueueType type) -> MetalCommandPool;

    /// Submits command buffers to a queue with fence signal and wait operations.
    auto Submit(QueueType type, std::span<MetalCommandBuffer*> command_buffers,
    ...
    auto WriteDescriptor(DescriptorHeapHandle heap, std::uint32_t index, const DescriptorWrite& write) -> void;
    ...
    auto operator=(const MetalDevice&) -> MetalDevice& = delete;
    MetalDevice(MetalDevice&&) noexcept;
    auto operator=(MetalDevice&&) noexcept -> MetalDevice&;
    ~MetalDevice();
};
